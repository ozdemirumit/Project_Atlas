...
export function RuntimeTrustPanel({ enablement }: { enablement: ConnectorCapabilityEnablement }) {
  ...
  return <section className="target-configuration-panel runtime-trust-panel" aria-labelledby="runtime-trust-title">
    ...
    {mutation.isError && <div className="workspace-message error-state" role="alert"><AlertTriangle size={20} /><div><h3>Runtime trust unavailable</h3><p>Enablement lineage, signed runtime controls, trust-policy evidence, freshness, requested scope, or separation of duties failed.</p></div></div>}
