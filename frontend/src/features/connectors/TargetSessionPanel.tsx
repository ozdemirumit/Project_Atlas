...
export function TargetSessionPanel({ activation }: { activation: ConnectorRuntimeActivation }) {
  ...
  return <><section className="target-configuration-panel target-session-panel" aria-labelledby="target-session-title">
    ...
    {mutation.isError && <div className="workspace-message error-state" role="alert"><AlertTriangle size={20} /><div><h3>Target session verification unavailable</h3><p>Runtime lineage, signed session-policy and network controls, TLS, target identity, read-only privilege, requested scope, or separation of duties failed.</p></div></div>}
