...
describe("ConnectorLifecycleOverview", () => {
  it("distinguishes platform coverage from instance authority", () => {
    ...
    expect(screen.getByLabelText("Delivery status")).toHaveTextContent(
      "12Available stages0In progressFinal recommendation dispositionLatest available capability",
    );
    ...
    expect(screen.getByText("Recommendation adjudication")).toBeVisible();
    expect(screen.getByText("Recommendation presentation")).toBeVisible();
    expect(screen.getByText("Recommendation draft promotion")).toBeVisible();
    expect(screen.getByText("Recommendation review readiness")).toBeVisible();
    expect(screen.getByText("Recommendation human-review request")).toBeVisible();
    expect(screen.getByText("Recommendation reviewer assignment")).toBeVisible();
    ...
    expect(screen.getByText("Recommendation track review decision")).toBeVisible();
    expect(screen.getByText("Recommendation correction resubmission")).toBeVisible();
    expect(screen.getAllByText("Final recommendation disposition")).toHaveLength(2);
