...
export function RuntimeActivationPanel({ brokerage }: { brokerage: ConnectorSecretBrokerageAuthorization }) {
  ...
  return <><section className="target-configuration-panel runtime-activation-panel" aria-labelledby="runtime-activation-title">
    ...
    {mutation.isError && <div className="workspace-message error-state" role="alert"><AlertTriangle size={20} /><div><h3>Runtime activation unavailable</h3><p>Brokerage lineage, signed activation-policy controls, runtime health evidence, requested scope, or separation of duties failed.</p></div></div>}
