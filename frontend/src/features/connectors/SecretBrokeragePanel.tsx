...
export function SecretBrokeragePanel({ runtimeTrust }: { runtimeTrust: ConnectorRuntimeTrustGrant }) {
  ...
  return <section className="target-configuration-panel secret-brokerage-panel" aria-labelledby="secret-brokerage-title">
    ...
    {mutation.isError && <div className="workspace-message error-state" role="alert"><AlertTriangle size={20} /><div><h3>Secret brokerage unavailable</h3><p>Runtime lineage, credential posture, signed delivery-policy evidence, requested scope, or separation of duties failed.</p></div></div>}
