...
export type WorkflowProtectedRuntimeContextUseAuthorizationAuthority = {
  protected_runtime_context_use_authority_granted: boolean;
...
export type WorkflowProtectedRuntimeContextUseAuthorization = {
  authorization_lease_id: string;
  state: "authorized_unconsumed";
  effective_state: "active" | "expired";
  issued_at: string;
...
export type WorkflowProtectedRuntimeContextUseAuthorizationInventory = {
  authorizations: WorkflowProtectedRuntimeContextUseAuthorization[];
  server_time: string;
  durable: true;
};
...
export type WorkflowProtectedRuntimeStartAuthorizationAuthority = {
  protected_runtime_start_authority_granted: boolean;
  protected_runtime_context_use_authority_granted: false;
  runtime_use_authorized: false;
  runtime_start_authorized: false;
  runtime_resume_authorized: false;
...
export type WorkflowProtectedRuntimeStartAuthorization = {
  authorization_lease_id: string;
  state: "authorized_unconsumed";
  effective_state: "active" | "expired";
  issued_at: string;
...
export type WorkflowProtectedRuntimeStartAuthorizationInventory = {
  authorizations: WorkflowProtectedRuntimeStartAuthorization[];
  server_time: string;
  durable: true;
};
...
export type WorkflowProtectedRuntimeProcessCreationAuthorizationAuthority = {
  protected_runtime_process_creation_authority_granted: boolean;
...
export type WorkflowProtectedRuntimeProcessCreationAuthorization = {
  authorization_lease_id: string;
  readiness_result_reference: string;
  state: "authorized_unconsumed";
  effective_state: "active" | "expired";
  issued_at: string;
  valid_until: string;
  ...
  policy_version: "1.0";
  process_creation_profile_reference: string;
  authority: WorkflowProtectedRuntimeProcessCreationAuthorizationAuthority;
  integrity_reference: string;
};

export type WorkflowProtectedRuntimeProcessCreationAuthorizationInventory = {
  authorizations: WorkflowProtectedRuntimeProcessCreationAuthorization[];
  server_time: string;
  durable: true;
};
...
export type WorkflowProtectedRuntimeProcessCreation = {
  ...
  completed_at: string | null;
  recorded_at: string | null;
  process_created: boolean | null;
  process_sealed: boolean | null;
  process_suspended: boolean | null;
  policy_reference: "policy.workflow-protected-runtime-process-creation-consumption:1.0";
  process_creation_profile_reference: string;
...
export type WorkflowProtectedRuntimeProcessCreationInventory = {
  process_creations: WorkflowProtectedRuntimeProcessCreation[];
  server_time: string;
  durable: true;
};
...
const protectedRuntimeContextUseAuthorizationAuthorityFields = [
...
const protectedRuntimeContextUseAuthorizationFields = [
  "authorization_lease_id",
  "state",
  "effective_state",
  "issued_at",
...
const protectedRuntimeStartAuthorizationAuthorityFields = [
...
const protectedRuntimeStartAuthorizationFields = [
  "authorization_lease_id",
  "state",
  "effective_state",
  "issued_at",
...
const protectedRuntimeStartAuthorizationInventoryFields = [
  "authorizations",
  "server_time",
  "durable",
] as const;
...
const protectedRuntimeProcessCreationAuthorizationAuthorityFields = [
...
const protectedRuntimeProcessCreationAuthorizationFields = [
  "authorization_lease_id",
...
const protectedRuntimeProcessCreationFields = [
  "process_creation_id",
  "attempt_state",
  "result_state",
  "started_at",
...
function hasProtectedRuntimeContextUseOnlyAuthority(
  value: unknown,
  useAuthorityGranted: boolean,
): value is WorkflowProtectedRuntimeContextUseAuthorizationAuthority {
  return (
    isObject(value) &&
    hasExactKeys(value, protectedRuntimeContextUseAuthorizationAuthorityFields) &&
    protectedRuntimeContextUseAuthorizationAuthorityFields.every((field) =>
      field === "protected_runtime_context_use_authority_granted"
        ? value[field] === useAuthorityGranted
        : value[field] === false,
    )
  );
}

function isProtectedRuntimeContextUseAuthorization(
  value: unknown,
  serverTime: string,
): value is WorkflowProtectedRuntimeContextUseAuthorization {
  if (
    !isObject(value) ||
    !hasExactKeys(value, protectedRuntimeContextUseAuthorizationFields) ||
    containsCredentialMaterial(value) ||
    !isTimezoneAwareTimestamp(value.issued_at) ||
    !isTimezoneAwareTimestamp(value.valid_until) ||
    !isTimezoneAwareTimestamp(value.effective_until)
  ...
  const evaluatedAt = Date.parse(serverTime);
  const expectedEffectiveState = evaluatedAt >= validUntil ? "expired" : "active";
  const useAuthorityGranted = expectedEffectiveState === "active";
  return (
    isStableIdentifier(value.authorization_lease_id) &&
    value.authorization_lease_id.startsWith(
      "workflow-protected-runtime-context-use-lease.",
    ) &&
    value.state === "authorized_unconsumed" &&
    value.effective_state === expectedEffectiveState &&
    issuedAt <= evaluatedAt &&
    ...
    value.consumer_contract_id ===
      "contract.workflow-protected-transport-target-context-capsule-consumer" &&
    value.consumer_contract_version === "1.0" &&
    value.purpose_id === "purpose.workflow-protected-runtime-context-use-evaluation" &&
    value.policy_id === "policy.workflow-protected-runtime-context-use-authorization" &&
    value.policy_version === "1.0" &&
    ...
    hasProtectedRuntimeContextUseOnlyAuthority(value.authority, useAuthorityGranted) &&
    isStableIdentifier(value.integrity_reference) &&
    value.integrity_reference.startsWith(
...
function hasProtectedRuntimeStartRequestOnlyAuthority(
  value: unknown,
  startRequestAuthorityGranted: boolean,
): value is WorkflowProtectedRuntimeStartAuthorizationAuthority {
  return (
    isObject(value) &&
    hasExactKeys(value, protectedRuntimeStartAuthorizationAuthorityFields) &&
    protectedRuntimeStartAuthorizationAuthorityFields.every((field) =>
      field === "protected_runtime_start_authority_granted"
        ? value[field] === startRequestAuthorityGranted
        : value[field] === false,
    )
  );
}

function isProtectedRuntimeStartAuthorization(
  value: unknown,
  serverTime: string,
): value is WorkflowProtectedRuntimeStartAuthorization {
  if (
    !isObject(value) ||
    !hasExactKeys(value, protectedRuntimeStartAuthorizationFields) ||
    containsCredentialMaterial(value) ||
    !isTimezoneAwareTimestamp(value.issued_at) ||
    !isTimezoneAwareTimestamp(value.valid_until) ||
    !isTimezoneAwareTimestamp(value.effective_until)
  ...
  return (
    isStableIdentifier(value.authorization_lease_id) &&
    value.authorization_lease_id.startsWith("workflow-protected-runtime-start-lease.") &&
    value.state === "authorized_unconsumed" &&
    value.effective_state === expectedEffectiveState &&
    issuedAt <= evaluatedAt &&
    issuedAt < validUntil &&
    ...
    hasProtectedRuntimeStartRequestOnlyAuthority(
      value.authority,
...
function hasProtectedRuntimeProcessCreationRequestOnlyAuthority(
  value: unknown,
  processCreationAuthorityGranted: boolean,
): value is WorkflowProtectedRuntimeProcessCreationAuthorizationAuthority {
  return (
    isObject(value) &&
    hasExactKeys(value, protectedRuntimeProcessCreationAuthorizationAuthorityFields) &&
    protectedRuntimeProcessCreationAuthorizationAuthorityFields.every((field) =>
      field === "protected_runtime_process_creation_authority_granted"
        ? value[field] === processCreationAuthorityGranted
        : value[field] === false,
    )
  );
}

function isProtectedRuntimeProcessCreationAuthorization(
  value: unknown,
  serverTime: string,
): value is WorkflowProtectedRuntimeProcessCreationAuthorization {
  if (
    !isObject(value) ||
    !hasExactKeys(value, protectedRuntimeProcessCreationAuthorizationFields) ||
    containsCredentialMaterial(value) ||
    !isTimezoneAwareTimestamp(value.issued_at) ||
    !isTimezoneAwareTimestamp(value.valid_until) ||
    !isTimezoneAwareTimestamp(value.effective_until)
  ...
  return (
    isStableIdentifier(value.authorization_lease_id) &&
    value.authorization_lease_id.startsWith(
      "workflow-protected-runtime-process-creation-lease.",
    ) &&
    isStableIdentifier(value.readiness_result_reference) &&
    value.readiness_result_reference.startsWith(
    ...
    value.consumer_contract_id ===
      "contract.workflow-protected-transport-target-context-capsule-consumer" &&
    value.consumer_contract_version === "1.0" &&
    value.purpose_id === "purpose.workflow-protected-runtime-process-creation-request" &&
    value.policy_id ===
      "policy.workflow-protected-runtime-process-creation-authorization" &&
    value.policy_version === "1.0" &&
    isStableIdentifier(value.process_creation_profile_reference) &&
    value.process_creation_profile_reference.startsWith(
      "integrity.workflow-protected-runtime-process-creation-profile.",
    ) &&
    hasProtectedRuntimeProcessCreationRequestOnlyAuthority(
      value.authority,
      expectedEffectiveState === "active",
    ) &&
    isStableIdentifier(value.integrity_reference) &&
...
function isProtectedRuntimeProcessCreation(
  value: unknown,
  serverTime: string,
): value is WorkflowProtectedRuntimeProcessCreation {
  if (
    !isObject(value) ||
    !hasExactKeys(value, protectedRuntimeProcessCreationFields) ||
    containsCredentialMaterial(value) ||
    !isTimezoneAwareTimestamp(value.started_at) ||
    (value.completed_at !== null && !isTimezoneAwareTimestamp(value.completed_at)) ||
    (value.recorded_at !== null && !isTimezoneAwareTimestamp(value.recorded_at))
  ...
  const pending =
    ...
    completedAt === null &&
    recordedAt === null &&
    value.process_created === null &&
    value.process_sealed === null &&
    value.process_suspended === null;
  const projectedUncertainty =
    value.result_state === "process_creation_outcome_uncertain" &&
    completedAt === null &&
    recordedAt !== null &&
    value.process_created === null &&
    value.process_sealed === null &&
    value.process_suspended === null;
  const success =
    value.result_state === "process_created_suspended_in_protected_boundary" &&
    completedAt !== null &&
    recordedAt !== null &&
    value.process_created === true &&
    value.process_sealed === true &&
    value.process_suspended === true;
  const knownWithoutCreation =
    (value.result_state === "process_creation_rejected_without_creation" ||
      value.result_state === "process_creation_failed_without_creation") &&
    completedAt !== null &&
    recordedAt !== null &&
    value.process_created === false &&
    value.process_sealed === false &&
    value.process_suspended === false;
  const durableUncertainty =
    ...
    value.process_suspended === null;
  return (
    isStableIdentifier(value.process_creation_id) &&
    value.process_creation_id.startsWith("prpc-consumption-") &&
    value.attempt_state === "process_creation_attempt_started" &&
    startedAt <= evaluatedAt &&
    (completedAt === null || (completedAt >= startedAt && completedAt <= evaluatedAt)) &&
    (recordedAt === null ||
      (recordedAt >= (completedAt ?? startedAt) && recordedAt <= evaluatedAt)) &&
    (pending || projectedUncertainty || success || knownWithoutCreation || durableUncertainty) &&
    value.policy_reference ===
...
export async function listWorkflowProtectedRuntimeContextUseAuthorizations(): Promise<WorkflowProtectedRuntimeContextUseAuthorizationInventory> {
  const response = await apiFetch(
  ...
  const data = await readData(
    response,
    "Workflow protected runtime-context use authorization retrieval failed",
  );
  if (
    !isObject(data) ||
    !hasExactKeys(data, protectedRuntimeContextUseAuthorizationInventoryFields) ||
    containsCredentialMaterial(data) ||
    !Array.isArray(data.authorizations) ||
    data.authorizations.length > 256 ||
    !isTimezoneAwareTimestamp(data.server_time) ||
    ...
    throw new ApiRequestError(
      "Workflow protected runtime-context use authorization response was unsafe",
      response.status,
    );
  }
  const serverTime = data.server_time;
  if (
    !data.authorizations.every((authorization) =>
      isProtectedRuntimeContextUseAuthorization(authorization, serverTime),
    )
  ) {
    throw new ApiRequestError(
      "Workflow protected runtime-context use authorization response was unsafe",
...
export async function listWorkflowProtectedRuntimeStartAuthorizations(): Promise<WorkflowProtectedRuntimeStartAuthorizationInventory> {
  const response = await apiFetch(
  ...
  const data = await readData(
    response,
    "Workflow protected runtime-start authorization retrieval failed",
  );
  if (
    !isObject(data) ||
    !hasExactKeys(data, protectedRuntimeStartAuthorizationInventoryFields) ||
    containsCredentialMaterial(data) ||
    !Array.isArray(data.authorizations) ||
    data.authorizations.length > 256 ||
    !isTimezoneAwareTimestamp(data.server_time) ||
    ...
    throw new ApiRequestError(
      "Workflow protected runtime-start authorization response was unsafe",
      response.status,
    );
  }
  const serverTime = data.server_time;
  if (
    !data.authorizations.every((authorization) =>
      isProtectedRuntimeStartAuthorization(authorization, serverTime),
    )
  ) {
    throw new ApiRequestError(
      "Workflow protected runtime-start authorization response was unsafe",
...
export async function listWorkflowProtectedRuntimeProcessCreationAuthorizations(): Promise<WorkflowProtectedRuntimeProcessCreationAuthorizationInventory> {
  const response = await apiFetch(
  ...
  const data = await readData(
    response,
    "Workflow protected runtime process-creation authorization retrieval failed",
  );
  if (
    !isObject(data) ||
    !hasExactKeys(data, protectedRuntimeProcessCreationAuthorizationInventoryFields) ||
    containsCredentialMaterial(data) ||
    !Array.isArray(data.authorizations) ||
    data.authorizations.length > 256 ||
    !isTimezoneAwareTimestamp(data.server_time) ||
    ...
    throw new ApiRequestError(
      "Workflow protected runtime process-creation authorization response was unsafe",
      response.status,
    );
  }
  const serverTime = data.server_time;
  if (
    !data.authorizations.every((authorization) =>
      isProtectedRuntimeProcessCreationAuthorization(authorization, serverTime),
    )
  ) {
    throw new ApiRequestError(
      "Workflow protected runtime process-creation authorization response was unsafe",
...
export async function listWorkflowProtectedRuntimeProcessCreations(): Promise<WorkflowProtectedRuntimeProcessCreationInventory> {
  ...
  const data = await readData(
    response,
    "Workflow protected runtime process-creation retrieval failed",
  );
  if (
    !isObject(data) ||
    !hasExactKeys(data, protectedRuntimeProcessCreationInventoryFields) ||
    containsCredentialMaterial(data) ||
    !Array.isArray(data.process_creations) ||
    data.process_creations.length > 256 ||
    !isTimezoneAwareTimestamp(data.server_time) ||
    data.durable !== true
  ) {
    throw new ApiRequestError(
      "Workflow protected runtime process-creation response was unsafe",
      response.status,
    );
  }
  const serverTime = data.server_time;
  if (
    !data.process_creations.every((processCreation) =>
      isProtectedRuntimeProcessCreation(processCreation, serverTime),
    )
  ) {
    throw new ApiRequestError(
      "Workflow protected runtime process-creation response was unsafe",
  ...
  const processCreationIds = new Set<string>();
  for (const processCreation of data.process_creations) {
    if (
      !isObject(processCreation) ||
      typeof processCreation.process_creation_id !== "string" ||
      processCreationIds.has(processCreation.process_creation_id)
    ) {
      throw new ApiRequestError(
        "Workflow protected runtime process-creation response was unsafe",
        response.status,
