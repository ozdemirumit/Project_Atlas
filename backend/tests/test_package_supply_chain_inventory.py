...
_DENIAL_PAYLOAD = {
    "source_validation_id": "validation.wiring-denied-0001",
    "source_validation_digest": "f" * 64,
    "package_digest": "f" * 64,
    "acknowledged_untrusted_package_content": True,
}
_DENIAL_ENDPOINT = "/api/v1/connectors/package-supply-chain-inventories"


def test_package_supply_chain_inventory_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            ...
            headers={"Idempotency-Key": "inventory-wiring-denied-0001"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/inventory.wiring-denied-0001")
...
def test_package_supply_chain_inventory_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            _DENIAL_ENDPOINT,
            json=_DENIAL_PAYLOAD,
            headers={**headers, "Idempotency-Key": "inventory-wiring-denied-0002"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/inventory.wiring-denied-0001", headers=headers)
