...
def _authorization_denied(actor: AuthenticatedSubject) -> AuthorizationService:
    """An `AuthorizationService` with the real permission catalog but zero `RoleAssignment`s --
    proving a real, logged-in identity is denied by the real service, not by a faked override."""
    permissions = (
        RECOMMENDATION_PROTECTED_INSPECTION_LEASE_CREATE,
        RECOMMENDATION_PROTECTED_INSPECTION_LEASE_READ,
        RECOMMENDATION_PROTECTED_CONTENT_PRESENTATION_CREATE,
...
async def test_recommendation_protected_content_requires_authentication() -> None:
    ...
    with TestClient(
        ...
        response = client.post(
            f"/api/v1/recommendations/{assignment.recommendation_id}/protected-inspections/leases",
            headers={"Idempotency-Key": "recommendation-content-http-denied-0001"},
            json={
                "source_assignment_set_id": assignment.assignment_set_id,
                "source_assignment_set_digest": assignment.canonical_digest,
                "track_code": track,
...
async def test_recommendation_protected_content_requires_permission() -> None:
    ...
    with TestClient(
        create_app(
            settings(),
            identity_provider=BasicTestIdentityProvider(actor),
            authorization_service=_authorization_denied(actor),
            recommendation_protected_inspection_service=inspection,
        ),
        base_url="https://testserver",
    ) as client:
        ...
        response = client.post(
            f"/api/v1/recommendations/{assignment.recommendation_id}/protected-inspections/leases",
            headers={
                "Idempotency-Key": "recommendation-content-http-denied-0002",
                "X-CSRF-Token": csrf,
            },
            json={
                "source_assignment_set_id": assignment.assignment_set_id,
