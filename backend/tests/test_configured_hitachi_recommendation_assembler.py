...
TARGET_ID = "asset.storage.real-device"
...
def _evidence(evidence_id: str, source_type: str) -> EvidenceUnit:
    return EvidenceUnit(
        ...
        collection_method="Hitachi Ops Center allowlisted C1 read",
        summary="A component reported a warning.",
        integrity="Digest-only evidence from an allowlisted C1 HTTPS GET response",
        completeness="A single current read.",
        quality_limitations=(),
        citation="Hitachi Ops Center read.",
    )
...
def _diagnostic_step(step_id: str) -> DiagnosticStep:
    return DiagnosticStep(
        step_id=step_id,
        question="Is the warning reproduced?",
        target_id=TARGET_ID,
...
def _active_finding_case() -> RcaCase:
    ...
    hypothesis = RcaHypothesis(
        ...
        statement="A controller condition may be contributing to the warning.",
        mechanism="A degraded component could reduce redundancy.",
        expected_affected_entities=(TARGET_ID,),
        expected_unaffected_entities=(),
        expected_sequence=("Condition begins.", "Warning observed."),
        ...
        confidence_rationale="A current direct warning supports the hypothesis.",
        diagnostic_steps=(_diagnostic_step("diagnostic.path-events"),),
    )
    return RcaCase(
        case_id="rca_real1",
        version=1,
        ...
        user_report="A controller reports a warning.",
        expected_behavior="Storage paths remain healthy.",
        ...
        symptoms=(
            NormalizedSymptom(
                ...
                statement="A controller reports a warning.",
                first_observed_at=NOW,
                current_state="observed warning",
        ...
        findings=(
            RcaFinding(
                ...
                cause_type=CauseType.CONTRIBUTING_CAUSE,
                statement="A controller condition is supported as a candidate contributing cause.",
                confirmation_level=ConfirmationLevel.SUPPORTED,
                evidence_references=(health.evidence_id,),
                residual_uncertainty=("No repeat read confirms persistence.",),
            ),
        ),
        assumptions=("The connector's read reflects the array's current state.",),
        unknowns=("Whether the warning is persistent or transient.",),
        conflicts=(),
        ...
        provisional_statement=ProvisionalCauseStatement(
            ...
            supporting_evidence=(health.evidence_id,),
            contradicting_evidence=(),
            residual_uncertainty=("The warned state has not been reproduced.",),
            alternatives_not_ruled_out=("Transient component state.",),
            prevention_or_verification_implication="Collect current path and event evidence.",
        ),
        ...
        component_versions=("rca-case-contract.v1", "hitachi-ops-center-connector.0.1.0"),
...
def _request() -> RecommendationRequest:
    return RecommendationRequest(
        ...
        target_id=TARGET_ID,
        decision_question="What should be done about the current warning?",
...
def test_build_populates_options_from_real_evidence_only() -> None:
    assembler = ConfiguredHitachiRecommendationAssembler()
    ...
    assert artifact.data_profile == "configured_hitachi_read_only"
