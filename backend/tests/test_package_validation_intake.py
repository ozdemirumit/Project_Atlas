...
def test_package_validation_api_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            "/api/v1/connectors/package-validations",
            json={
                "source_acquisition_id": "acquisition.wiring-denied-0001",
                "source_acquisition_digest": "f" * 64,
                "package_digest": "f" * 64,
                "acknowledged_untrusted_quarantined_package": True,
            },
            headers={"Idempotency-Key": "package-validation-denied-0001"},
        )
        read = client.get("/api/v1/connectors/package-validations/validation.wiring-denied-0001")
...
def test_package_validation_api_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            ...
            headers={
                "X-CSRF-Token": csrf,
                "Idempotency-Key": "package-validation-denied-0002",
            },
        )
        read = client.get(
            "/api/v1/connectors/package-validations/validation.wiring-denied-0001",
