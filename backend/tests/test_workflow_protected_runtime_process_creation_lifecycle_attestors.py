...
from atlas.modules.workflows.adapters import (
    protected_runtime_process_creation_lifecycle_attestors as lifecycle_attestors,
)
...
DevelopmentAttestor = lifecycle_attestors.DeterministicDevelopmentWorkflowProtectedRuntimeProcessCreationLifecycleAttestor  # noqa: E501
UnavailableAttestor = (
    lifecycle_attestors.UnavailableWorkflowProtectedRuntimeProcessCreationLifecycleAttestor
)
AuthorizationError = (
...
def _request(
    *,
    ...
    for field in fields(LifecycleRequest):
        name = field.name
        ...
        if policy_value is not None:
            ...
            values[name] = "1.0"
        elif name == "runtime_readiness_result_state":
            values[name] = "runtime_ready_in_protected_boundary"
    ...
    return LifecycleRequest(**cast(Any, values))


@pytest.mark.asyncio
async def test_development_attestor_binds_ready_lineage_and_zero_effects() -> None:
    ...
    first = await attestor.attest_runtime_process_creation_lifecycle(request)
    second = await attestor.attest_runtime_process_creation_lifecycle(request)
    ...
    assert first.exact_readiness_result_confirmed is True
    assert first.runtime_started_confirmed is True
    assert first.runtime_ready_confirmed is True
    assert first.readiness_assessment_confirmed is True
    ...
    assert first.generic_process_created is False
    assert first.scheduling_performed is False
    assert first.prior_process_creation_claim_absent is True
    assert first.prior_process_creation_lease_absent is True
    ...
    assert attestor.verify_runtime_process_creation_lifecycle_attestation(first) is True
    assert (
        attestor.verify_runtime_process_creation_lifecycle_attestation(
            replace(first, request_nonce_digest="d" * 64)
        )
        is False
    )
...
@pytest.mark.asyncio
async def test_development_attestor_preserves_negative_lifecycle_evidence() -> None:
    attestor = DevelopmentAttestor(
        development_enabled=True,
        clock=lambda: NOW,
        ...
        process_creation_absent=False,
        scheduling_absent=False,
        competing_process_creation_authorization_absent=False,
        competing_process_creation_attempt_absent=False,
    )

    evidence = await attestor.attest_runtime_process_creation_lifecycle(_request())

    assert evidence.runtime_started_confirmed is False
    assert evidence.runtime_ready_confirmed is False
    assert evidence.readiness_assessment_confirmed is False
    assert evidence.runtime_envelope_started is False
    assert evidence.destination_fence_current is False
    assert evidence.generic_process_created is True
    assert evidence.scheduling_performed is True
    assert evidence.prior_process_creation_claim_absent is False
    assert evidence.prior_process_creation_lease_absent is False
    assert attestor.verify_runtime_process_creation_lifecycle_attestation(evidence) is True


@pytest.mark.asyncio
async def test_development_attestor_binds_every_ready_result_lineage_digest() -> None:
    attestor = DevelopmentAttestor(
        development_enabled=True,
        clock=lambda: NOW,
    )
    ...
    for name in lineage_digest_fields:
        tampered = replace(evidence, **cast(Any, {name: "0" * 64}))
        assert attestor.verify_runtime_process_creation_lifecycle_attestation(tampered) is False
...
async def test_development_attestor_rejects_naive_clock() -> None:
    ...
    with pytest.raises(AuthorizationError) as caught:
        await naive.attest_runtime_process_creation_lifecycle(_request())
    assert (
        caught.value.code
        == "workflow_protected_runtime_process_creation_lifecycle_clock_must_be_aware"
    )
...
async def test_unavailable_and_disabled_attestors_fail_closed_without_request_leakage() -> None:
    ...
    for attestor in (disabled, unavailable):
        with pytest.raises(AuthorizationError) as caught:
            await attestor.attest_runtime_process_creation_lifecycle(request)
        ...
        assert (
            attestor.verify_runtime_process_creation_lifecycle_attestation(cast(Any, None)) is False
        )
