...
from atlas.modules.connectors.vendors.huawei_dorado.manifest import CONTROLLER_HEALTH_CAPABILITY_ID
...
def _evidence(evidence_id: str, source_type: str) -> EvidenceUnit:
    return EvidenceUnit(
        ...
        collection_method="Huawei Dorado allowlisted C1 read",
        summary="A controller reported a non-normal health status.",
        integrity="Digest-only evidence from an allowlisted C1 DeviceManager response",
        completeness="A single current read.",
        quality_limitations=(),
        citation="Huawei Dorado read.",
    )
...
def _active_finding_case() -> RcaCase:
    ...
    hypothesis = RcaHypothesis(
        ...
        statement="A controller condition may be contributing to the finding.",
        mechanism="A faulty controller could reduce redundancy.",
    ...
    return RcaCase(
        ...
        user_report="A controller reports a non-normal health status.",
        expected_behavior="Storage controllers remain healthy.",
        ...
        symptoms=(
            NormalizedSymptom(
                ...
                statement="A controller reports a non-normal health status.",
        ...
        impact_scope=ImpactScope(
            ...
            current_impact="A controller finding is observed.",
        ...
        assumptions=("The connector's read reflects the system's current state.",),
        ...
        provisional_statement=ProvisionalCauseStatement(
            ...
            alternatives_not_ruled_out=("Transient controller state.",),
        ...
        component_versions=("rca-case-contract.v1", "huawei-dorado-connector.0.1.0"),
        data_profile="configured_huawei_dorado_read_only",
...
def test_build_populates_options_from_real_evidence_only() -> None:
    assembler = ConfiguredHuaweiDoradoRecommendationAssembler()
    ...
    assert artifact.data_profile == "configured_huawei_dorado_read_only"
    ...
    assert any(
        step.capability_id == CONTROLLER_HEALTH_CAPABILITY_ID
        for option in artifact.options
