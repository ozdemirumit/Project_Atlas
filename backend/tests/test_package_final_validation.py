...
async def test_human_eligibility_does_not_require_fixed_assurance(
    ...
    subject = replace(final_operator(), authentication_method=method, assurance_level=assurance)

    report = await final_validate(service, lab, policy, subject=subject)

    assert report.outcome is FinalValidationOutcome.ELIGIBLE


@pytest.mark.asyncio
async def test_final_validation_rejects_non_human_actor() -> None:
    service, lab, policy = await final_fixture()
    subject = replace(final_operator(), kind=SubjectKind.SERVICE)

    with pytest.raises(PackageFinalValidationError, match="package_final_human_required"):
        await final_validate(service, lab, policy, subject=subject)
...
def test_final_validation_api_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            "/api/v1/connectors/package-final-validations",
            json={
                "source_lab_self_test_id": "lab-self-test.wiring-denied-0001",
                "source_lab_self_test_digest": "f" * 64,
                "package_digest": "f" * 64,
                "policy_id": "policy.wiring-denied-0001",
                "policy_digest": "f" * 64,
                "acknowledged_evidence_only_no_approval": True,
            },
            headers={"Idempotency-Key": "final-validation-denied-0001"},
        )
        read = client.get(
            "/api/v1/connectors/package-final-validations/final-validation.wiring-denied-0001"
        )
...
def test_final_validation_api_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            ...
            headers={
                "X-CSRF-Token": csrf,
                "Idempotency-Key": "final-validation-denied-0002",
            },
        )
        read = client.get(
            "/api/v1/connectors/package-final-validations/final-validation.wiring-denied-0001",
