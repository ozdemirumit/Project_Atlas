...
_DENIAL_PAYLOAD = {
    "source_approval_request_id": "request.wiring-denied-0001",
    "source_approval_request_digest": "f" * 64,
    "package_digest": "f" * 64,
    "publisher_claim_id": "claim.wiring-denied-0001",
    "publisher_claim_digest": "f" * 64,
    "attestation_policy_id": "policy.wiring-denied-0001",
    "attestation_policy_digest": "f" * 64,
    "purpose": "Prove that a real unprivileged identity is denied, not faked.",
    "acknowledged_attestation_grants_no_lifecycle_authority": True,
}
_DENIAL_ENDPOINT = "/api/v1/connectors/publisher-attestations"


def test_publisher_attestation_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            ...
            headers={"Idempotency-Key": "attest-wiring-denied-0001"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/report.wiring-denied-0001")
...
def test_publisher_attestation_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            _DENIAL_ENDPOINT,
            json=_DENIAL_PAYLOAD,
            headers={**headers, "Idempotency-Key": "attest-wiring-denied-0002"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/report.wiring-denied-0001", headers=headers)
