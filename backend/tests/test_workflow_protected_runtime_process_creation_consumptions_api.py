...
ENDPOINT = "/api/v1/workflows/protected-runtime-process-creation-consumptions"
...
def _attempt(suffix: str) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        attempt_id=f"prpc-attempt-{suffix}",
        consumption_id=f"prpc-consumption-{suffix}",
...
def _result(
    ...
    state: WorkflowProtectedRuntimeProcessCreationConsumptionResultState,
) -> SimpleNamespace:
    created: bool | None = (
        state
        is (
...
def _presentations() -> tuple[SimpleNamespace, ...]:
    states = WorkflowProtectedRuntimeProcessCreationConsumptionResultState
    pending = _attempt("a" * 24)
    created = _attempt("b" * 24)
    ...
    return (
        ...
        SimpleNamespace(
            attempt=created,
            result=_result(created, states.PROCESS_CREATED_SUSPENDED_IN_PROTECTED_BOUNDARY),
        ),
        SimpleNamespace(
            attempt=rejected,
            result=_result(rejected, states.PROCESS_CREATION_REJECTED_WITHOUT_CREATION),
        ),
        SimpleNamespace(
            attempt=failed,
            result=_result(failed, states.PROCESS_CREATION_FAILED_WITHOUT_CREATION),
        ),
        SimpleNamespace(
            attempt=uncertain,
            result=_result(uncertain, states.PROCESS_CREATION_OUTCOME_UNCERTAIN),
        ),
    )
...
class _ConflictService(_Service):
    async def consume(self, **kwargs: Any) -> SimpleNamespace:
        ...
        raise WorkflowProtectedRuntimeProcessCreationConsumptionError(
            "protected_runtime_process_creation_outcome_uncertain_no_retry"
        )
...
def _payload() -> dict[str, object]:
    ...
    return {
        ...
        "idempotency_key": "process-creation-consumption-api-0001",
    }
...
def _exact_workload() -> tuple[Any, str]:
    return _workload_service_and_token(
        identity_id=WORKFLOW_PROTECTED_RUNTIME_PROCESS_CREATOR_SUBJECT,
        audience=WORKFLOW_PROTECTED_RUNTIME_PROCESS_CREATOR_AUDIENCE,
    )


def test_process_creation_consumption_has_distinct_read_only_scope() -> None:
    settings = _settings()
    ...
    assert WORKFLOW_PROTECTED_RUNTIME_PROCESS_CREATION_CONSUMPTION_READ == (
        "workflow.protected-runtime-process-creation-consumptions.read"
    )
    assert scope.resource_id == "resource.workflow.protected-runtime-process-creation-consumptions"
    assert scope.capability_class.value == "C1"
    assert any(
        assignment.assignment_id
        == "assignment.development.workflow-protected-runtime-process-creation-consumptions"
        and assignment.scope == scope
        for assignment in cast(Any, service)._assignments
    )


def test_password_get_is_minimized_no_store_and_has_all_four_terminal_states() -> None:
    service = _Service()
    app = create_app(
        _settings(),
        workflow_protected_runtime_process_creation_consumption_service=cast(Any, service),
    )
    ...
    items = response.json()["data"]["process_creations"]
    ...
    assert items[-1]["process_created"] is None
    assert items[-1]["process_sealed"] is None
    assert items[-1]["process_suspended"] is None
...
def test_only_exact_workload_can_post_strict_acknowledged_request() -> None:
    ...
    headers = _workload_headers(token, WORKFLOW_PROTECTED_RUNTIME_PROCESS_CREATOR_AUDIENCE)
    with TestClient(app) as client:
        csrf = _login(client)
    ...
    assert service.calls[0]["consumer_subject_id"] == (
        WORKFLOW_PROTECTED_RUNTIME_PROCESS_CREATOR_SUBJECT
    )
