...
ENDPOINT = "/api/v1/workflows/protected-runtime-process-resume-authorizations"
...
def _lease(*, scope: WorkflowScope = SCOPE) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        authorization_lease_id=(
            "workflow-protected-runtime-process-resume-authorization-lease.0123456789abcdef01234567"
        ),
        process_scheduling_result_id=(
            "workflow-protected-runtime-process-scheduling-result.imp-230"
        ),
        canonical_digest="c" * 64,
        scope=scope,
        state=(WorkflowProtectedRuntimeProcessResumeAuthorizationLeaseState.AUTHORIZED_UNCONSUMED),
        issued_at=NOW,
...
class _Service:
    ...
    def _presentation(self, lease: SimpleNamespace) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            ...
            protected_runtime_process_resume_authority_granted=active,
        )
...
class _FailingService(_Service):
    async def authorize(self, **kwargs: Any) -> SimpleNamespace:
        ...
        raise WorkflowProtectedRuntimeProcessResumeAuthorizationError(
            "workflow_protected_runtime_process_resume_repository_unavailable"
        )
...
def _payload() -> dict[str, object]:
    ...
    return {
        ...
        "idempotency_key": "runtime-process-resume-authorization-api-0001",
    }
...
def test_read_authorization_has_dedicated_permission_c1_scope_and_assignment() -> None:
    ...
    assert WORKFLOW_PROTECTED_RUNTIME_PROCESS_RESUME_AUTHORIZATION_READ == (
        "workflow.protected-runtime-process-resume-authorizations.read"
    )
    assert scope.resource_id == (
        "resource.workflow.protected-runtime-process-resume-authorizations"
    )
    assert scope.capability_class.value == "C1"
    assignments = cast(Any, service)._assignments
    assert any(
        assignment.assignment_id
        == "assignment.development.workflow-protected-runtime-process-resume-authorizations"
...
def test_exact_workload_post_and_password_session_get_are_minimized_without_mfa() -> None:
    ...
    app = create_app(
        ...
        workflow_protected_runtime_process_resume_authorization_service=cast(Any, service),
    )
    ...
    assert item["process_scheduling_result_reference"] == (
        "integrity.workflow-protected-runtime-process-scheduling-result."
        f"{sha256(cast(str, _payload()['process_scheduling_result_id']).encode('utf-8')).hexdigest()[:24]}"  # noqa: E501
    )
    ...
    assert item["authority"]["protected_runtime_process_resume_authority_granted"] is True
    assert item["authority"]["runtime_resume_authorized"] is False
...
def test_active_and_expired_inventory_projects_authority_from_server_time() -> None:
    ...
    assert active_item["authority"]["protected_runtime_process_resume_authority_granted"]
    assert active_item["authority"]["runtime_resume_authorized"] is False
...
def test_strict_post_rejects_operational_material_and_false_acknowledgements_before_io() -> None:
    ...
    forbidden_inputs: tuple[tuple[str, object], ...] = (
        ("process_scheduling_result_digest", "a" * 64),
        ("ttl_seconds", 300),
...
def test_human_personal_token_ai_and_wrong_workload_boundary_fail_before_io() -> None:
    ...
    for identity_id, audience in (
        ...
        isolated_app = create_app(
            _settings(),
            workload_identity_service=workload_service,
            workflow_protected_runtime_process_resume_authorization_service=cast(
...
def test_default_inventory_fails_closed_without_a_durable_service() -> None:
    ...
    assert type(service).__name__ == ("WorkflowProtectedRuntimeProcessResumeAuthorizationService")
    ...
    assert response.json()["code"] == (
        "workflow_protected_runtime_process_resume_authorization_service_unavailable"
    )
    _assert_no_store(response)
