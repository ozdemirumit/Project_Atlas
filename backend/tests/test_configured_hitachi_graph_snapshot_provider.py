...
INSTANCE_ID = "connector-instance.hitachi-graph"
...
) -> tuple[ConfiguredHitachiGraphSnapshotProvider, SyntheticHitachiTransport]:
    transport = SyntheticHitachiTransport(routes or {})
    provider = ConfiguredHitachiGraphSnapshotProvider(
...
def _configuration() -> SimpleNamespace:
    return SimpleNamespace(
        configuration_id="connection_configuration.hitachi-graph",
...
async def test_snapshot_is_empty_without_one_configured_instance() -> None:
    ...
    assert "single active configured Hitachi MCP" in snapshot.known_gaps[0]
...
@pytest.mark.asyncio
async def test_snapshot_is_empty_without_allowlisted_storage() -> None:
    provider, transport = build_provider(
        configurations=(_configuration(),),
        instances=(_instance(),),
    )
    ...
    assert snapshot.entities == ()
    assert "No active Hitachi storage serial number" in snapshot.known_gaps[0]
    assert transport.requests == []
...
async def test_snapshot_maps_real_inventory_into_storage_system_entities_with_no_relationships() -> (
    ...
    assert snapshot.data_profile == "configured_hitachi_read_only"
    assert len(snapshot.entities) == 2
    assert all(entity.entity_type is EntityType.STORAGE_SYSTEM for entity in snapshot.entities)
    assert snapshot.relationships == ()
    ...
    assert transport.requests == ["/v1/objects/storages"]
    assert any("no CMDB or hypervisor connector" in gap for gap in snapshot.known_gaps)
