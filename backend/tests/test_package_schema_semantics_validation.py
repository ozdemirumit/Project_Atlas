...
_DENIAL_PAYLOAD = {
    "source_content_policy_scan_id": "scan.wiring-denied-0001",
    "source_content_policy_scan_digest": "f" * 64,
    "package_digest": "f" * 64,
    "acknowledged_untrusted_schema_content": True,
}
_DENIAL_ENDPOINT = "/api/v1/connectors/package-schema-semantics-validations"


def test_package_schema_semantics_validation_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            ...
            headers={"Idempotency-Key": "schema-wiring-denied-0001"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/validation.wiring-denied-0001")
...
def test_package_schema_semantics_validation_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            _DENIAL_ENDPOINT,
            json=_DENIAL_PAYLOAD,
            headers={**headers, "Idempotency-Key": "schema-wiring-denied-0002"},
        )
        read = client.get(f"{_DENIAL_ENDPOINT}/validation.wiring-denied-0001", headers=headers)
