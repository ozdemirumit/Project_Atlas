...
from atlas.modules.workflows.adapters import (
    protected_runtime_process_resume_state_attestors as state_attestors,
)
from atlas.modules.workflows.application import (
    protected_runtime_process_resume_authorization_ports as scheduling_ports,
)
from atlas.modules.workflows.domain import (
    protected_runtime_process_resume_authorization_domain as scheduling_domain,
)
from atlas.modules.workflows.domain.models import WorkflowScope, canonical_digest

DevelopmentAttestor = (
    state_attestors.DeterministicDevelopmentWorkflowProtectedRuntimeProcessResumeStateAttestor
)
UnavailableAttestor = state_attestors.UnavailableWorkflowProtectedRuntimeProcessResumeStateAttestor
AuthorizationError = scheduling_ports.WorkflowProtectedRuntimeProcessResumeAuthorizationError
StateRequest = scheduling_ports.WorkflowProtectedRuntimeProcessResumeStateAttestationRequest
code_owned_policy = (
    scheduling_domain.code_owned_workflow_protected_runtime_process_resume_authorization_policy
)
...
async def test_development_attestor_binds_suspended_process_and_zero_effects() -> None:
    ...
    first = await attestor.attest_runtime_process_resume_state(request)
    second = await attestor.attest_runtime_process_resume_state(request)

    assert first == second
    assert first.request_nonce_digest == request.request_nonce_digest
    assert first.exact_process_scheduling_result_confirmed is True
    ...
    assert first.process_scheduled_confirmed is True
    assert first.process_not_runnable_confirmed is True
    ...
    assert first.prior_process_resume_claim_absent is True
    assert first.prior_process_resume_lease_absent is True
    assert first.pending_or_conflicting_resume_absent is True
    assert first.pending_or_conflicting_dispatch_absent is True
    assert first.pending_or_conflicting_execution_absent is True
    assert first.pending_or_conflicting_supervision_absent is True
    assert first.pending_or_conflicting_stop_absent is True
    assert first.pending_or_conflicting_cleanup_absent is True
    assert first.pending_or_conflicting_replacement_absent is True
    assert first.pending_or_conflicting_rescheduling_absent is True
    ...
    assert attestor.verify_runtime_process_resume_state_attestation(first) is True
    assert (
        attestor.verify_runtime_process_resume_state_attestation(
...
async def test_development_attestor_preserves_negative_process_state() -> None:
    attestor = DevelopmentAttestor(
        ...
        prior_resume_claim_absent=False,
        prior_resume_lease_absent=False,
        pending_or_conflicting_resume_absent=False,
        pending_or_conflicting_cleanup_absent=False,
        scheduling_performed=True,
        resume_performed=True,
        dispatch_performed=True,
        execution_performed=True,
    )

    evidence = await attestor.attest_runtime_process_resume_state(_request())
    ...
    assert evidence.process_scheduled_confirmed is False
    assert evidence.process_not_runnable_confirmed is False
    ...
    assert evidence.prior_process_resume_claim_absent is False
    assert evidence.prior_process_resume_lease_absent is False
    assert evidence.pending_or_conflicting_resume_absent is False
    assert evidence.pending_or_conflicting_cleanup_absent is False
    assert attestor.verify_runtime_process_resume_state_attestation(evidence) is True
...
async def test_development_attestor_binds_every_lineage_digest() -> None:
    ...
    for name in digest_fields:
        ...
        assert attestor.verify_runtime_process_resume_state_attestation(tampered) is False
...
async def test_unavailable_disabled_and_naive_attestors_fail_closed() -> None:
    ...
    for attestor in (naive, disabled, unavailable):
        with pytest.raises(AuthorizationError) as caught:
            await attestor.attest_runtime_process_resume_state(request)
        ...
        assert attestor.verify_runtime_process_resume_state_attestation(cast(Any, None)) is False
