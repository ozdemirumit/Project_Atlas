...
def test_lab_self_test_api_requires_authentication() -> None:
    ...
    with TestClient(create_app(Settings(environment="test"))) as client:
        created = client.post(
            "/api/v1/connectors/package-lab-self-tests",
            json={
                "source_runner_validation_id": "runner-validation.wiring-denied-0001",
                "source_runner_validation_digest": "f" * 64,
                "package_digest": "f" * 64,
                "lab_plan_id": "lab-plan.wiring-denied-0001",
                "lab_plan_digest": "f" * 64,
                "acknowledged_non_production_read_only_lab_access": True,
            },
            headers={"Idempotency-Key": "lab-self-test-denied-0001"},
        )
        read = client.get(
            "/api/v1/connectors/package-lab-self-tests/lab-self-test.wiring-denied-0001"
        )
...
def test_lab_self_test_api_requires_permission() -> None:
    ...
    with TestClient(create_app(_settings(development_role_ids=()))) as client:
        ...
        created = client.post(
            ...
            headers={
                "X-CSRF-Token": csrf,
                "Idempotency-Key": "lab-self-test-denied-0002",
            },
        )
        read = client.get(
            "/api/v1/connectors/package-lab-self-tests/lab-self-test.wiring-denied-0001",
