...
def test_assignment_api_requires_permission_for_every_endpoint() -> None:
    """`test_assignment_api_rejects_secret_input_and_minimizes_response` above already proves a
    genuinely unauthenticated caller is denied (401) and that a missing CSRF header is denied
    (403 csrf_validation_failed). Neither proves that a real, logged-in, but unprivileged subject
    is denied by the real `AuthorizationService` -- this test drives that case at every
    credential-assignment endpoint (list, options, create, read-by-id), each carrying a real CSRF
    ...
    with TestClient(create_app(settings(development_role_ids=()))) as client:
        ...
        options = client.get(
            f"{endpoint}/options",
            params={"source_target_binding_id": "connector-target-configuration.denied"},
        )
        created = client.post(
            endpoint,
            json={},
            headers={"Idempotency-Key": "cred-api-denied-0001", "X-CSRF-Token": csrf},
        )
        read = client.get(f"{endpoint}/connector-credential-assignment.denied")
