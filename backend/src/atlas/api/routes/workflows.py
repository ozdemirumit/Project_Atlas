...
from atlas.api.security import (
    ...
    authorize_workflow_protected_runtime_start_consumption_read,
...
from atlas.api.workflow_schemas import (
    ...
    CreateWorkflowProtectedRuntimeStartConsumptionInput,
    ...
    WorkflowProtectedRuntimeReadinessAuthorizationResponse,
    WorkflowProtectedRuntimeReadinessConsumptionData,
    WorkflowProtectedRuntimeReadinessConsumptionInventoryData,
    WorkflowProtectedRuntimeReadinessConsumptionInventoryResponse,
    WorkflowProtectedRuntimeReadinessConsumptionResponse,
    ...
    WorkflowProtectedRuntimeStartConsumptionInventoryData,
    WorkflowProtectedRuntimeStartConsumptionInventoryResponse,
    WorkflowProtectedRuntimeStartConsumptionResponse,
...
@router.get(
    "/protected-runtime-start-consumptions",
    response_model=WorkflowProtectedRuntimeStartConsumptionInventoryResponse,
)
async def list_workflow_protected_runtime_start_consumptions(
    request: Request,
    response: Response,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
    decision: Annotated[
        AuthorizationDecision,
        Depends(authorize_workflow_protected_runtime_start_consumption_read),
    ],
) -> WorkflowProtectedRuntimeStartConsumptionInventoryResponse:
    del decision
    _no_store(response)
    ...
    service = cast(
        WorkflowProtectedRuntimeStartConsumptionService,
        request.app.state.workflow_protected_runtime_start_consumption_service,
    )
    try:
        presentations = await service.list_presentations(scope=scope, limit=256)
        server_time = await service.repository.get_authoritative_time()
        ...
        if len(consumption_ids) != len(set(consumption_ids)) or any(
            ...
            raise RuntimeError("protected runtime-start consumption scope mismatch")
        data = WorkflowProtectedRuntimeStartConsumptionInventoryData(
            starts=[
                WorkflowProtectedRuntimeStartConsumptionData.from_domain(
                    presentation.attempt,
                    presentation.result,
                    evaluated_at=server_time,
                )
        ...
        raise AtlasError(
            status=503,
            code="workflow_protected_runtime_start_consumption_service_unavailable",
            title="Protected runtime-start outcomes unavailable",
            detail="Runtime-start outcome metadata is temporarily unavailable.",
            retryable=True,
        ) from error
    return WorkflowProtectedRuntimeStartConsumptionInventoryResponse(
        data=data,
        meta=_meta(request),
    )
...
@router.post(
    "/protected-runtime-start-consumptions",
    response_model=WorkflowProtectedRuntimeStartConsumptionResponse,
    status_code=201,
)
async def create_workflow_protected_runtime_start_consumption(
    payload: CreateWorkflowProtectedRuntimeStartConsumptionInput,
    request: Request,
    response: Response,
    subject: Annotated[
        AuthenticatedSubject,
    ...
    try:
        ...
        data = WorkflowProtectedRuntimeStartConsumptionData.from_domain(
            presentation.attempt,
            presentation.result,
        )
    except WorkflowProtectedRuntimeStartConsumptionError as error:
        ...
        raise AtlasError(
            ...
            code=(
                "workflow_protected_runtime_start_outcome_uncertain"
                if uncertain
                else (
            ...
            title=(
                "Protected runtime-start outcome uncertain"
                if uncertain
                else ("Protected runtime-start unavailable" if unavailable else "Request denied")
            ),
            detail=(
                "The start outcome is uncertain and this request must not be retried."
                if uncertain
                else (
                    "Runtime-start processing is temporarily unavailable."
                    if unavailable
                    else "The current identity or evidence is not authorized for this operation."
                )
            ),
        ...
        raise AtlasError(
            status=503,
            code="workflow_protected_runtime_start_consumption_service_unavailable",
            title="Protected runtime-start unavailable",
            detail="Runtime-start processing is temporarily unavailable.",
            retryable=True,
        ) from error
    return WorkflowProtectedRuntimeStartConsumptionResponse(
        data=data,
        meta=_meta(request),
    )
...
@router.get(
    ...
    response_model=WorkflowProtectedRuntimeReadinessAuthorizationInventoryResponse,
)
async def list_workflow_protected_runtime_readiness_authorizations(
    request: Request,
    response: Response,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
    decision: Annotated[
        AuthorizationDecision,
        Depends(authorize_workflow_protected_runtime_readiness_authorization_read),
    ],
) -> WorkflowProtectedRuntimeReadinessAuthorizationInventoryResponse:
    del decision
    _no_store(response)
    ...
    service = cast(
        WorkflowProtectedRuntimeReadinessAuthorizationService,
        request.app.state.workflow_protected_runtime_readiness_authorization_service,
    )
    try:
        presentations = await service.list_presentations(scope=scope, limit=256)
        server_time = await service.repository.get_authoritative_time()
        if any(presentation.lease.scope != scope for presentation in presentations):
            raise RuntimeError("protected runtime-readiness authorization scope mismatch")
        inventory_data = WorkflowProtectedRuntimeReadinessAuthorizationInventoryData(
            authorizations=[
                WorkflowProtectedRuntimeReadinessAuthorizationData.from_domain(presentation)
                for presentation in presentations
            ],
            server_time=server_time,
            durable=service.durable,
        ...
        raise AtlasError(
            status=503,
            code="workflow_protected_runtime_readiness_authorization_service_unavailable",
            title="Protected runtime-readiness authorization unavailable",
            detail="Runtime-readiness authorization metadata is temporarily unavailable.",
            retryable=True,
        ) from error
    return WorkflowProtectedRuntimeReadinessAuthorizationInventoryResponse(
        data=inventory_data,
        meta=_meta(request),
    )
...
@router.post(
    "/protected-runtime-readiness-authorizations",
    response_model=WorkflowProtectedRuntimeReadinessAuthorizationResponse,
    status_code=201,
)
async def create_workflow_protected_runtime_readiness_authorization(
    payload: CreateWorkflowProtectedRuntimeReadinessAuthorizationInput,
    request: Request,
    response: Response,
    subject: Annotated[
        AuthenticatedSubject,
    ...
    try:
        lease = await service.authorize(
            ...
            start_result_digest=payload.start_result_digest,
            policy_id=payload.policy_id,
            policy_version=payload.policy_version,
            idempotency_key=payload.idempotency_key,
            context=WorkflowProtectedTransportTargetContextCapsuleHandoffAuthorizationContext(
                ...
                correlation_id=str(request.state.correlation_id),
                decision_id=(
                    "decision.workflow-protected-runtime-readiness-consumer-authenticated"
                ),
                requested_at=datetime.now(UTC),
            ),
        )
        presentations = await (
            service.repository.list_protected_runtime_readiness_authorization_presentations(
                scope=scope,
                authorization_lease_ids=(lease.authorization_lease_id,),
                limit=1,
            )
        ...
        if len(presentations) != 1:
            raise RuntimeError("runtime-readiness authorization projection unavailable")
        presentation = presentations[0]
        if presentation.lease.canonical_digest != lease.canonical_digest:
            raise RuntimeError("runtime-readiness authorization projection mismatch")
        data = WorkflowProtectedRuntimeReadinessAuthorizationData.from_domain(presentation)
    except WorkflowProtectedRuntimeReadinessAuthorizationError as error:
        conflict = any(
            marker in error.code
            for marker in (
                "already_authorized",
        ...
        raise AtlasError(
            ...
            code=(
                "authorization_denied"
                if conflict
                else "workflow_protected_runtime_readiness_authorization_service_unavailable"
            ),
            title=(
                "Request denied"
                if conflict
                else "Protected runtime-readiness authorization unavailable"
            ),
            detail=(
                "The current identity or evidence is not authorized for this operation."
                if conflict
                else "Runtime-readiness authorization metadata is temporarily unavailable."
            ),
            retryable=not conflict,
        ) from error
    except Exception as error:
    ...
    return WorkflowProtectedRuntimeReadinessAuthorizationResponse(
        data=data,
        meta=_meta(request),
    )
...
@router.get(
    "/protected-runtime-readiness-consumptions",
    response_model=WorkflowProtectedRuntimeReadinessConsumptionInventoryResponse,
)
async def list_workflow_protected_runtime_readiness_consumptions(
    request: Request,
    response: Response,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
    decision: Annotated[
        AuthorizationDecision,
        Depends(authorize_workflow_protected_runtime_readiness_consumption_read),
    ],
) -> WorkflowProtectedRuntimeReadinessConsumptionInventoryResponse:
    del decision
    _no_store(response)
    ...
    service = cast(
        WorkflowProtectedRuntimeReadinessConsumptionService,
        request.app.state.workflow_protected_runtime_readiness_consumption_service,
    )
    try:
        presentations = await service.list_presentations(scope=scope, limit=256)
        server_time = await service.repository.get_authoritative_time()
        ...
        if len(consumption_ids) != len(set(consumption_ids)) or any(
            ...
            raise RuntimeError("protected runtime-readiness consumption scope mismatch")
        data = WorkflowProtectedRuntimeReadinessConsumptionInventoryData(
            readiness=[
                WorkflowProtectedRuntimeReadinessConsumptionData.from_domain(
                    presentation,
                    evaluated_at=server_time,
                )
                for presentation in presentations
        ...
        raise AtlasError(
            status=503,
            code="workflow_protected_runtime_readiness_consumption_service_unavailable",
            title="Protected runtime-readiness outcomes unavailable",
            detail="Runtime-readiness outcome metadata is temporarily unavailable.",
            retryable=True,
        ) from error
    return WorkflowProtectedRuntimeReadinessConsumptionInventoryResponse(
        data=data,
        meta=_meta(request),
    )
...
@router.post(
    "/protected-runtime-readiness-consumptions",
    response_model=WorkflowProtectedRuntimeReadinessConsumptionResponse,
    status_code=201,
)
async def create_workflow_protected_runtime_readiness_consumption(
    payload: CreateWorkflowProtectedRuntimeReadinessConsumptionInput,
    request: Request,
    response: Response,
    subject: Annotated[
        AuthenticatedSubject,
    ...
    try:
        ...
        data = WorkflowProtectedRuntimeReadinessConsumptionData.from_domain(presentation)
    except WorkflowProtectedRuntimeReadinessConsumptionError as error:
        uncertain = "uncertain" in error.code
        unavailable = "unavailable" in error.code or error.code.endswith(
            "durable_repository_required"
        )
        raise AtlasError(
            status=409 if uncertain else (503 if unavailable else 409),
            code=(
                "workflow_protected_runtime_readiness_outcome_uncertain"
                if uncertain
                else (
            ...
            title=(
                "Protected runtime-readiness outcome uncertain"
                if uncertain
                else (
                    "Protected runtime-readiness unavailable" if unavailable else "Request denied"
                )
            ),
            detail=(
                "The readiness outcome is uncertain and this request must not be retried."
                if uncertain
                else (
                    "Runtime-readiness processing is temporarily unavailable."
                    if unavailable
                    else "The current identity or evidence is not authorized for this operation."
                )
            ),
        ...
        raise AtlasError(
            status=503,
            code="workflow_protected_runtime_readiness_consumption_service_unavailable",
            title="Protected runtime-readiness unavailable",
            detail="Runtime-readiness processing is temporarily unavailable.",
            retryable=True,
        ) from error
