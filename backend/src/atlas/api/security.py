...
async def workflow_protected_runtime_process_scheduler_subject(
    request: Request,
    authorization: Annotated[
        str | None, Header(alias="Authorization", min_length=1, max_length=8192)
    ] = None,
    ...
    """Authenticate only the exact ADR-180 protected scheduler workload."""
    ...
    valid_envelope = (
        ...
        and scheme.lower() == "workload"
        and bool(token)
        and audience == WORKFLOW_PROTECTED_RUNTIME_PROCESS_SCHEDULER_AUDIENCE
        and environment_id == expected_environment
    )
    service: WorkloadIdentityService = request.app.state.workload_identity_service
    try:
    ...
    if (
        subject.kind is not SubjectKind.SERVICE
        or subject.authentication_method is not AuthenticationMethod.WORKLOAD_TOKEN
        or subject.subject_id != WORKFLOW_PROTECTED_RUNTIME_PROCESS_SCHEDULER_SUBJECT
    ):
        raise AtlasError(
            status=401,
            code="workload_authentication_failed",
...
async def authorize_workflow_protected_runtime_context_use_authorization_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-170 minimized inventory for a normal human session."""
    ...
    decision = await service.evaluate(
        AuthorizationRequest(
            ...
            resource_type="resource.workflow.protected-runtime-context-use-authorization",
...
async def authorize_workflow_protected_runtime_context_use_authorization_consumption_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-171 minimized inventory for a normal human session."""
    ...
    decision = await service.evaluate(
        AuthorizationRequest(
            ...
            resource_type=(
                "resource.workflow.protected-runtime-context-use-authorization-consumption"
            ),
...
async def authorize_workflow_protected_runtime_start_authorization_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-173 minimized inventory for a normal human session."""
    ...
    decision = await service.evaluate(
        AuthorizationRequest(
            ...
            resource_type="resource.workflow.protected-runtime-start-authorization",
...
async def authorize_workflow_protected_runtime_start_consumption_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-174 minimized inventory for a normal human session."""
    ...
    decision = await service.evaluate(
        AuthorizationRequest(
            ...
            resource_type="resource.workflow.protected-runtime-start-consumption",
...
async def authorize_workflow_protected_runtime_process_creation_authorization_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-177 minimized inventory for a normal human session."""
...
async def authorize_workflow_protected_runtime_process_scheduling_consumption_read(
    request: Request,
    subject: Annotated[AuthenticatedSubject, Depends(browser_session_subject)],
) -> AuthorizationDecision:
    """Authorize the ADR-180 minimized inventory for a normal human session."""
...
async def authorize_workflow_protected_runtime_context_use_read(
    ...
    """Authorize the ADR-172 minimized inventory for a normal human session."""
    ...
    decision = await service.evaluate(
        AuthorizationRequest(
            ...
            resource_type="resource.workflow.protected-runtime-context-use",
