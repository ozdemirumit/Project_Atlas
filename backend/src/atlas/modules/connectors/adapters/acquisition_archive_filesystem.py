...
class FileSystemAcquiredPackagePublisher:
    ...
    def _publish(self, package_digest: str, content: bytes) -> bool:
        ...
        try:
            ...
            if destination.exists() and self._read(package_digest, len(content)) == content:
                return False
            raise PackageAcquisitionError("package_acquisition_archive_conflict") from None
        except PackageAcquisitionError:
            raise
        except OSError as error:
            raise PackageAcquisitionError("package_acquisition_archive_publish_failed") from error
        finally:
    ...
    def _read(self, package_digest: str, size_bytes: int) -> bytes:
        ...
        try:
            ...
            raise PackageAcquisitionError("package_acquisition_archive_not_found") from error
        if (
    ...
    def _mkdir_safe(path: Path) -> None:
        for candidate in (path, *path.parents):
            if candidate.is_symlink():
                raise PackageAcquisitionError("package_acquisition_archive_path_unsafe")
