from __future__ import annotations

import asyncio

from atlas.modules.connectors.domain.license_analysis import (
    ConnectorPackageLicenseAnalysis,
    LicensePolicySnapshot,
)


class InMemoryPackageLicenseAnalysisRepository:
    durable = False

    def __init__(self) -> None:
        self._records: dict[str, ConnectorPackageLicenseAnalysis] = {}
        self._lock = asyncio.Lock()

    async def get_by_id(self, *, analysis_id: str) -> ConnectorPackageLicenseAnalysis | None:
        return self._records.get(analysis_id)

    async def get_by_source_analysis(
        self, *, source_malware_analysis_id: str
    ) -> ConnectorPackageLicenseAnalysis | None:
        return next(
            (
                item
                for item in self._records.values()
                if item.source_malware_analysis_id == source_malware_analysis_id
            ),
            None,
        )

    async def get_by_create_key(
        self, *, analyzed_by: str, idempotency_key: str
    ) -> ConnectorPackageLicenseAnalysis | None:
        return next(
            (
                item
                for item in self._records.values()
                if item.analyzed_by == analyzed_by and item.idempotency_key == idempotency_key
            ),
            None,
        )

    async def add(self, analysis: ConnectorPackageLicenseAnalysis) -> bool:
        async with self._lock:
            if analysis.analysis_id in self._records:
                return False
            if any(
                item.source_malware_analysis_id == analysis.source_malware_analysis_id
                or (
                    item.analyzed_by == analysis.analyzed_by
                    and item.idempotency_key == analysis.idempotency_key
                )
                for item in self._records.values()
            ):
                return False
            self._records[analysis.analysis_id] = analysis
            return True

    async def close(self) -> None:
        return None


class StaticLicensePolicySnapshotProvider:
    def __init__(self, snapshot: LicensePolicySnapshot) -> None:
        self._snapshot = snapshot

    async def current(self, *, organization_id: str, environment_id: str) -> LicensePolicySnapshot:
        return self._snapshot
