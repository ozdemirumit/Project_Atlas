...
class ConnectorInvocationAuthorizationService:
    ...
    async def create(
        ...
        *,
        ...
        profile = await self._profile_source.get_by_id(profile_id=invocation_profile_id)
        envelope = await self._envelope_source.get_by_id(envelope_id=input_envelope_id)
        policy = await self._policy_source.get_by_id(policy_id=authorization_policy_id)
        ...
        async with self._mutation_lock:
            prior = await self._repository.get_by_target_session(
            ...
            seed = self._digest(
                [source.verification_id, capability.capability_id, envelope.canonical_digest]
            )
...
def build_connector_invocation_profile(
    *,
    ...
    snapshot = ConnectorInvocationProfileSnapshot(
        profile_id="connector-invocation-profile.development-read-only",
...
def build_connector_invocation_input_envelope(
    *, profile: ConnectorInvocationProfileSnapshot, issued_at: datetime, expires_at: datetime
    ...
    snapshot = ConnectorInvocationInputEnvelopeSnapshot(
        envelope_id="connector-invocation-input-envelope.development-empty",
