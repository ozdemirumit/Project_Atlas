...
class HuaweiDoradoHttpsTransport:
    """Endpoint-bound HTTPS transport for one OceanStor Dorado system, implementing the vendor's
    real session lifecycle internally. Mirrors HitachiOpsCenterHttpsTransport's security posture
    exactly (hostname/path validation, enforced TLS verification, no redirects, bounded response
    size, rejected non-standard JSON constants). Unlike Hitachi and Brocade, every `get()` call
    performs its own bounded login -> read -> logout cycle (see ports.py for why), so this
    transport never holds a session open longer than one read.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(system_id, str) or not _SYSTEM_ID.fullmatch(system_id):
            raise ValueError("system_id must be a fixed alphanumeric identifier")
        if (
        ...
        self._session_path = f"/deviceManager/rest/{system_id}/sessions"
        self._credential_provider = credential_provider
        # Divided across the three physical requests one logical read performs (login, read,
        # logout) so the whole bounded operation cannot exceed the configured budget by 3x.
    ...
    def _bounded_read(self, path: str) -> Mapping[str, object]:
        username, password = self._credential_pair()
        token, cookie = self._login(username, password)
        try:
            _status, _headers, body = self._raw_request(
                "GET",
                path,
                body=None,
                extra_headers={"iBaseToken": token, "Cookie": cookie},
            )
        finally:
            self._logout(token, cookie)
        return self._decoded_object(body)

    def _login(self, username: str, password: str) -> tuple[str, str]:
        payload = json.dumps(
            {"username": username, "password": password, "scope": "0"},
            sort_keys=True,
            separators=(",", ":"),
        ).encode("utf-8")
        _status, headers, body = self._raw_request(
            "POST", self._session_path, body=payload, extra_headers=None
        )
        decoded = self._decoded_object(body)
        data = decoded.get("data")
        token = data.get("iBaseToken") if isinstance(data, dict) else None
        ...
        return token, self._session_cookie(headers)

    def _logout(self, token: str, cookie: str) -> None:
        # Best-effort: the read already succeeded or failed on its own terms; a session that
        # cannot be explicitly closed will still expire server-side. Never let logout mask the
        # real outcome of the read it followed.
        with contextlib.suppress(HuaweiTransportError):
            self._raw_request(
                "DELETE",
                self._session_path,
                body=json.dumps({"iBaseToken": token}).encode("utf-8"),
                extra_headers={"iBaseToken": token, "Cookie": cookie},
            )
    ...
    def _decoded_object(self, body: bytes) -> Mapping[str, object]:
        try:
    ...
    def _session_cookie(headers: Mapping[str, str]) -> str:
        # Only the name=value pair is echoed back; cookie attributes (Path, Domain, Expires,
        # HttpOnly) are intentionally not parsed since this transport never persists a cookie
        # beyond the single bounded read that received it.
        raw = headers.get("Set-Cookie")
        if not raw:
            raise HuaweiTransportError(
                "vendor_permission_denied",
                "The vendor did not return a usable session cookie.",
                retryable=False,
            )
        pair = raw.split(";", 1)[0].strip()
        if not _COOKIE_PAIR.fullmatch(pair):
            raise HuaweiTransportError(
                "vendor_permission_denied",
                "The vendor did not return a usable session cookie.",
                retryable=False,
    ...
    @staticmethod
    def _http_status_error(status: int) -> HuaweiTransportError:
        if status in {401, 403}:
            return HuaweiTransportError(
                "vendor_permission_denied",
    ...
    @staticmethod
    def _connection_error(error: BaseException) -> HuaweiTransportError:
