...
_SESSION_PATH = "/api/v2/aa/sessions"
...
class HuaweiPacificHttpsTransport:
    """Endpoint-bound HTTPS transport for one OceanStor Pacific cluster, implementing the
    vendor's real session lifecycle internally. Mirrors HuaweiDoradoHttpsTransport's security
    posture and login -> read -> logout cycle exactly, but simpler: Pacific's confirmed auth uses
    only a header token (no cookie), and its endpoints carry no per-system path segment.
    """
    ...
    def _bounded_read(self, path: str) -> Mapping[str, object]:
        ...
        try:
            _status, _headers, body = self._raw_request(
                "GET", path, body=None, extra_headers={"X-Auth-Token": token}
            )
        finally:
            self._logout(token)
    ...
    def _login(self, username: str, password: str) -> str:
        payload = json.dumps(
            {"user_name": username, "password": password, "isEncrypt": False, "scope": "0"},
            sort_keys=True,
            separators=(",", ":"),
        ).encode("utf-8")
        _status, _headers, body = self._raw_request(
            "POST", _SESSION_PATH, body=payload, extra_headers=None
        )
        decoded = self._decoded_object(body)
        data = decoded.get("data")
        token = data.get("x_auth_token") if isinstance(data, dict) else None
    ...
    def _logout(self, token: str) -> None:
        with contextlib.suppress(HuaweiPacificTransportError):
            self._raw_request(
                "DELETE", _SESSION_PATH, body=None, extra_headers={"X-Auth-Token": token}
            )
    ...
    def _raw_request(
        ...
        *,
        ...
        except HuaweiPacificTransportError:
            raise
    ...
    @staticmethod
    def _http_status_error(status: int) -> HuaweiPacificTransportError:
        if status in {401, 403}:
            return HuaweiPacificTransportError(
                "vendor_permission_denied",
    ...
    @staticmethod
    def _connection_error(error: BaseException) -> HuaweiPacificTransportError:
