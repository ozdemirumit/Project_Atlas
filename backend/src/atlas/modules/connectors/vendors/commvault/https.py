...
_LOGIN_PATH = "/webservice/Login"
_LOGOUT_PATH = "/webservice/Logout"
_AUTH_HEADER = "Authtoken"
...
class CommvaultHttpsTransport:
    """Endpoint-bound HTTPS transport for one Commvault CommServe, implementing the vendor's real
    session lifecycle internally. Mirrors HuaweiPacificHttpsTransport's login -> read -> logout
    cycle and security posture, with two confirmed real differences: the session token is
    returned in the login response *body* (field `token`), not a header, and it is presented on
    subsequent requests via the `Authtoken` header, not a vendor-specific token header or
    ...
    def _bounded_read(self, path: str) -> Mapping[str, object]:
        ...
        try:
            _status, body = self._raw_request(
                "GET", path, body=None, extra_headers={_AUTH_HEADER: token}
            )
        finally:
            self._logout(token)
    ...
    def _login(self, username: str, password: str) -> str:
        ...
        _status, body = self._raw_request("POST", _LOGIN_PATH, body=payload, extra_headers=None)
    ...
    def _logout(self, token: str) -> None:
        with contextlib.suppress(CommvaultTransportError):
            self._raw_request("POST", _LOGOUT_PATH, body=None, extra_headers={_AUTH_HEADER: token})
    ...
    def _raw_request(
        ...
        *,
        ...
        except CommvaultTransportError:
            raise
    ...
    @staticmethod
    def _http_status_error(status: int) -> CommvaultTransportError:
        if status in {401, 403}:
            return CommvaultTransportError(
                "vendor_permission_denied",
    ...
    @staticmethod
    def _connection_error(error: BaseException) -> CommvaultTransportError:
