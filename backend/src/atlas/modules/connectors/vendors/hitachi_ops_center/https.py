...
class HitachiOpsCenterHttpsTransport:
    """Endpoint-bound, pre-authenticated HTTPS transport for read-only vendor requests."""
    ...
    async def get(self, path: str) -> Mapping[str, object]:
        ...
        return await asyncio.to_thread(self._get_sync, validated_path)

    def _get_sync(self, path: str) -> Mapping[str, object]:
        request = urllib.request.Request(
    ...
    @staticmethod
    def _http_status_error(status: int) -> HitachiTransportError:
        if status in {401, 403}:
            return HitachiTransportError(
                "vendor_permission_denied",
    ...
    @staticmethod
    def _connection_error(error: BaseException) -> HitachiTransportError:
