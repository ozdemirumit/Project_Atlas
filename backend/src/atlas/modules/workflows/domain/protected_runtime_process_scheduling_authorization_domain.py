...
from .protected_runtime_process_creation_consumption_domain import (
...
WORKFLOW_PROTECTED_RUNTIME_PROCESS_SCHEDULING_MAXIMUM_ATTESTATION_FRESHNESS_SECONDS = 1


class WorkflowProtectedRuntimeProcessSchedulingAuthorizationLeaseState(StrEnum):
    AUTHORIZED_UNCONSUMED = "authorized_unconsumed"


@dataclass(frozen=True, slots=True)
class WorkflowProtectedRuntimeProcessSchedulingAuthorizationAuthority:
    """A non-bearer future scheduling-request lease with no operational authority."""
    ...
    def __post_init__(self) -> None:
        ...
        declarations.pop("protected_runtime_process_scheduling_authority_granted")
        if any(declarations.values()):
            raise ValueError(
                "runtime process scheduling authorization grants operational authority"
            )
...
@dataclass(frozen=True, slots=True)
class WorkflowProtectedRuntimeProcessSchedulingAuthorizationPolicy:
    policy_id: str
    policy_version: str
    ...
    def __post_init__(self) -> None:
        ...
        if any(getattr(self, name) != value for name, value in expected.items()):
            raise ValueError("runtime process scheduling authorization policy is not code-owned")
        if self.canonical_digest != canonical_digest(self.digest_payload()):
            raise ValueError("runtime process scheduling authorization policy digest mismatch")
    ...
    def maximum_attestation_freshness_seconds(self) -> int:
        return WORKFLOW_PROTECTED_RUNTIME_PROCESS_SCHEDULING_MAXIMUM_ATTESTATION_FRESHNESS_SECONDS


def code_owned_workflow_protected_runtime_process_scheduling_authorization_policy_values() -> dict[
    str, object
]:
    source = code_owned_workflow_protected_runtime_process_creation_consumption_policy()
    profile = {
        "profile_id": "profile.workflow-protected-runtime-process-scheduling-request",
        "profile_version": "1.0",
    ...
    return {
        ...
        "required_source_state": (
            WorkflowProtectedRuntimeProcessCreationConsumptionResultState
        ).PROCESS_CREATED_SUSPENDED_IN_PROTECTED_BOUNDARY.value,
        "scheduling_profile_id": cast(str, profile["profile_id"]),
        "scheduling_profile_version": cast(str, profile["profile_version"]),
        "scheduling_profile_digest": canonical_digest(profile),
...
@lru_cache(maxsize=1)
def code_owned_workflow_protected_runtime_process_scheduling_authorization_policy() -> (
    WorkflowProtectedRuntimeProcessSchedulingAuthorizationPolicy
):
    values = code_owned_workflow_protected_runtime_process_scheduling_authorization_policy_values()
    return WorkflowProtectedRuntimeProcessSchedulingAuthorizationPolicy(
        **cast(Any, values), canonical_digest=canonical_digest(values)
    )


@dataclass(frozen=True, slots=True)
class WorkflowProtectedRuntimeProcessSchedulingAuthorizationClaim:
    claim_id: str
    ...
    process_creation_result_state: WorkflowProtectedRuntimeProcessCreationConsumptionResultState
    process_creation_failure_class: (
        WorkflowProtectedRuntimeProcessCreationConsumptionFailureClass | None
    )
    process_creation_outcome_known: bool
    ...
    process_creation_completed_at: datetime
    process_creation_result_recorded_at: datetime
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.claimed_at < self.process_creation_result_recorded_at
            or self.authority.protected_runtime_process_scheduling_authority_granted is not False
        ):
            raise ValueError("runtime process scheduling authorization claim is invalid")
...
@dataclass(frozen=True, slots=True)
class WorkflowProtectedRuntimeProcessSchedulingAuthorizationLease:
    ...
    state: WorkflowProtectedRuntimeProcessSchedulingAuthorizationLeaseState
    authority: WorkflowProtectedRuntimeProcessSchedulingAuthorizationAuthority
    canonical_digest: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or not self.process_creation_result_recorded_at <= self.issued_at < self.valid_until
            ...
            or self.scheduling_profile_id != policy.scheduling_profile_id
            or self.scheduling_profile_version != policy.scheduling_profile_version
            ...
            or self.authority.protected_runtime_process_scheduling_authority_granted is not True
        ):
            raise ValueError("runtime process scheduling authorization lease is invalid")
        _require_canonical_digest(self)
    ...
    def is_active(self, *, evaluated_at: datetime, consumed: bool = False) -> bool:
        if evaluated_at.tzinfo is None:
            raise ValueError("runtime process scheduling lease evaluation time must be aware")
    ...
    def presented_authority(
        ...
        return WorkflowProtectedRuntimeProcessSchedulingAuthorizationAuthority()


def _validate_source_snapshot(
    instance: WorkflowProtectedRuntimeProcessSchedulingAuthorizationClaim
    ...
    authority.pop("protected_runtime_process_scheduling_authority_granted")
    required_state = (
        WorkflowProtectedRuntimeProcessCreationConsumptionResultState
    ).PROCESS_CREATED_SUSPENDED_IN_PROTECTED_BOUNDARY
    if (
        instance.process_creation_result_state is not required_state
        or instance.process_creation_result_state.value != policy.required_source_state
        or instance.process_creation_failure_class is not None
        or instance.process_creation_outcome_known is not True
        ...
        or instance.process_creation_profile_id != source_policy.process_creation_profile_id
        or instance.process_creation_profile_version
        != source_policy.process_creation_profile_version
        or instance.process_creation_profile_digest != source_policy.process_creation_profile_digest
        ...
        or instance.process_creation_completed_at.tzinfo is None
        or instance.process_creation_result_recorded_at.tzinfo is None
        or instance.process_creation_result_recorded_at < instance.process_creation_completed_at
        or any(authority.values())
    ):
        raise ValueError("runtime process scheduling authorization source is ineligible")
...
def _require_identifiers(instance: object, names: tuple[str, ...]) -> None:
    for name in names:
        ...
        if (
            ...
            raise ValueError(f"runtime process scheduling authorization {name} is invalid")
...
def _require_canonical_digest(
    ...
    if instance.canonical_digest != canonical_digest(instance.digest_payload()):
        raise ValueError("runtime process scheduling authorization digest mismatch")
