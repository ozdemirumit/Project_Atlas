...
from atlas.core.persistence.models import WorkflowIdempotencyModel, WorkflowRunPlanModel
...
class PostgreSQLWorkflowPlanRepository:
    ...
    async def list_scoped(
        ...
        *,
        ...
        async with self._sessions() as session:
            return tuple(
                self._plan_from_row(row) for row in (await session.scalars(statement)).all()
            )
        ...
        *,
        ...
        async with self._sessions() as session:
            claim = await self._load_claim(
                session,
                scope=scope,
                creator_subject_id=creator_subject_id,
            ...
            return WorkflowPlanIdempotencyRecord(
                ...
                plan=self._plan_from_claim(claim),
            )
    ...
    async def _replay_result(
        ...
        *,
        ...
        if claim is None:
            return None
        claimed_plan = self._plan_from_claim(claim)
        status = (
        ...
        return cast(
            ...
            await session.scalar(
                select(WorkflowIdempotencyModel).where(
                    WorkflowIdempotencyModel.operation == "create",
                    WorkflowIdempotencyModel.idempotency_scope_id
                    == cls._idempotency_scope(scope, creator_subject_id),
                    ...
                    WorkflowIdempotencyModel.creator_subject_id == creator_subject_id,
                )
            ),
        )

    @classmethod
    def _plan_from_claim(cls, claim: WorkflowIdempotencyModel) -> WorkflowRunPlan:
        ...
        if (
            ...
            or claim.creator_subject_id != plan.creator_subject_id
            or claim.idempotency_scope_id
            != cls._idempotency_scope(plan.scope, plan.creator_subject_id)
        ):
            cls._contract_violation()
        return plan

    @classmethod
    def _plan_from_row(cls, row: WorkflowRunPlanModel) -> WorkflowRunPlan:
        try:
