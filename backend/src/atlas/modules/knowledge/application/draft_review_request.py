...
class OperationalKnowledgeReviewRequestOption:
...
class OperationalKnowledgeReviewRequestService:
        ...
        request_binding_digest = self._digest(
            {
                ...
                "review_request_option_id": review_request_option_id,
    ...
    async def list_requests(
        ...
        *,
        ...
        visible.sort(key=lambda item: item.review_request_id)
        await self._audit(
            actor,
            correlation_id,
            "operational_knowledge_review_request_inventory_listed",
            source_draft_id or self._environment_id,
    ...
    async def list_options(
        ...
        *,
        ...
        source_draft_id: str,
        correlation_id: str,
    ) -> tuple[OperationalKnowledgeReviewRequestOption, ...]:
        self._require_human(actor)
        if not self._adapter.available:
            await self._audit_options(actor, correlation_id, source_draft_id, 0)
            return ()
        try:
            source = await self._source.review_request_source(
                draft_id=source_draft_id,
        ...
        if claim is not None:
            ...
            await self._audit_options(actor, correlation_id, source.draft_id, 0)
            return ()
        options: list[OperationalKnowledgeReviewRequestOption] = []
        now = self._clock()
        ...
        for policy in policies:
            ...
            except OperationalKnowledgeReviewRequestError:
                continue
            options.append(self._option(source, policy))
        options.sort(key=lambda item: item.review_request_option_id)
        await self._audit_options(actor, correlation_id, source.draft_id, len(options))
        ...
        *,
        ...
        for policy in policies:
            try:
                ...
                continue
            if self._option_id(source, policy) != review_request_option_id:
                continue
            self._verify_source(
                source=source,
                policy=policy,
    ...
    async def _current_record(
        self, record: OperationalKnowledgeReviewRequestRecord
    ) -> OperationalKnowledgeReviewRequestRecord:
        ...
        if claim is None:
            raise OperationalKnowledgeReviewRequestError(
                "operational_knowledge_review_request_claim_not_found"
            )
        self._verify_claim(claim)
        try:
            source = await self._source.review_request_source(
        ...
        policy = await self._policy_source.get_by_id_in_scope(
            policy_id=record.orchestration_policy_id,
            organization_id=record.organization_id,
            environment_id=record.environment_id,
        )
        if policy is None:
        ...
        if (
            ...
            or claim.source_draft_digest != record.source_draft_digest
            ...
            or source.draft_id != record.source_draft_id
            or source.canonical_digest != record.source_draft_digest
            ...
            or source.draft_domain != record.draft_domain
            or source.content_type != record.content_type
            or source.language != record.language
            ...
            or source.draft_content_digest != record.draft_content_digest
            or source.draft_metadata_digest != record.draft_metadata_digest
            or source.provenance_digest != record.provenance_digest
            or policy.canonical_digest != record.orchestration_policy_digest
            or policy.organization_id != record.organization_id
            or policy.environment_id != record.environment_id
            or policy.policy_version != record.orchestration_policy_version
            or policy.required_adapter_id != record.orchestration_adapter_id
            or policy.domain_track_code != record.domain_track_code
            or policy.security_track_code != record.security_track_code
            or policy.domain_queue_id != record.domain_queue_id
            or policy.security_queue_id != record.security_queue_id
            or policy.assignment_strategy != record.assignment_strategy
    ...
    def _option(
        ...
        return OperationalKnowledgeReviewRequestOption(
            review_request_option_id=cls._option_id(source, policy),
            source_draft_id=source.draft_id,
            source_draft_digest=source.canonical_digest,
            knowledge_item_id=source.knowledge_item_id,
            ...
            orchestration_policy_expires_at=policy.expires_at,
    ...
    def _option_id(
        ...
        return f"operational-knowledge-review-request-option.{digest[:24]}"
    ...
    async def _audit_options(
        ...
        await self._audit(
            actor,
            correlation_id,
            "operational_knowledge_review_request_options_listed",
            source_draft_id,
            (("count", str(count)),),
            permission_id=KNOWLEDGE_DRAFT_REVIEW_REQUEST_READ,
        )
