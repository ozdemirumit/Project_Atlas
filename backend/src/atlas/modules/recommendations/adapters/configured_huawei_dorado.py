...
from atlas.modules.connectors.vendors.huawei_dorado.manifest import (
    CONTROLLER_FAILOVER_PLAN_CAPABILITY_ID,
    CONTROLLER_HEALTH_CAPABILITY_ID,
    PATH_EVENTS_CAPABILITY_ID,
    PATH_REMEDIATION_PLAN_CAPABILITY_ID,
)
...
class ConfiguredHuaweiDoradoRecommendationAssembler:
    """Builds the same five decision-support option categories as
    ConfiguredHitachiRecommendationAssembler (investigate, escalate, defer, restoration planning,
    remediation planning) from a real `RcaCase` produced by ConfiguredHuaweiDoradoRcaAssembler.
    ...
    identical to Hitachi's -- only the vendor-facing capability ids and text differ. Needs no
    connector access itself; the source case is already real by the time it reaches here.
    """
        ...
        return RecommendationOption(
            ...
            plan_steps=(
                ...
                PlanStep(
                    ...
                    2,
                    "diagnosis",
                    "Repeat the exact target controller-health observation.",
                    CONTROLLER_HEALTH_CAPABILITY_ID,
                    "C1",
                    "A reproducible or absent finding on the same target.",
                    "Stop if the target or controller mapping changes.",
                    False,
                ),
                PlanStep(
        ...
        return RecommendationOption(
            ...
            plan_steps=(
                PlanStep(
                    ...
                    1,
                    "monitoring",
                    "Continue the approved bounded controller-health observation until expiry.",
                    CONTROLLER_HEALTH_CAPABILITY_ID,
                    "C1",
