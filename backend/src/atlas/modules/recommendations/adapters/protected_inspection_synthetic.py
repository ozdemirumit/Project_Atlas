...
class SyntheticRecommendationProtectedInspectionBroker:
    broker_id = "recommendation-protected-inspection-broker.synthetic"
    attestor_id = "subject.recommendation-protected-inspection-broker-attestor"
    receipt_schema = "atlas.recommendation-protected-inspection-receipt.v1"
    ...
    async def issue(
        ...
        return RecommendationProtectedInspectionBrokerGrant(
            receipt=receipt,
            lease_secret=lease_secret,
        )


class UnavailableRecommendationProtectedInspectionBroker(RecommendationProtectedInspectionBroker):
    async def issue(
        self, instruction: RecommendationProtectedInspectionInstruction
    ) -> RecommendationProtectedInspectionBrokerGrant:
        del instruction
        raise RecommendationProtectedInspectionError(
            "recommendation_protected_inspection_broker_unavailable"
        )
