...
from atlas.core.persistence.models import (
    RecommendationHumanReviewFindingClaimModel,
    RecommendationHumanReviewFindingModel,
)
...
class PostgreSQLRecommendationHumanReviewFindingRepository:
    def __init__(
        self,
        *,
        engine: AsyncEngine,
    ...
    @classmethod
    def from_url(cls, database_url: str) -> PostgreSQLRecommendationHumanReviewFindingRepository:
    ...
    async def get(self, *, finding_packet_id: str) -> RecommendationHumanReviewFindingRecord | None:
        async with self._sessions() as session:
            row = await session.get(RecommendationHumanReviewFindingModel, finding_packet_id)
    ...
    async def get_by_source_presentation(
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(RecommendationHumanReviewFindingModel).where(
                    RecommendationHumanReviewFindingModel.source_presentation_id
    ...
    async def get_claim_by_source_presentation(
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(RecommendationHumanReviewFindingClaimModel).where(
                    RecommendationHumanReviewFindingClaimModel.source_presentation_id
                    == source_presentation_id
                )
            )
    ...
    async def get_claim_by_idempotency(
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(RecommendationHumanReviewFindingClaimModel).where(
                    RecommendationHumanReviewFindingClaimModel.claimed_by_subject_digest
                    == claimed_by_subject_digest,
                    RecommendationHumanReviewFindingClaimModel.idempotency_digest
                    == idempotency_digest,
                )
            )
            return self._claim_to_domain(row.payload) if row else None
    ...
    async def claim(self, claim: RecommendationHumanReviewFindingClaim) -> bool:
        payload = RecommendationHumanReviewFindingService._normalize(asdict(claim))
    ...
    async def add(self, record: RecommendationHumanReviewFindingRecord) -> bool:
        payload = RecommendationHumanReviewFindingService._normalize(asdict(record))
        assert isinstance(payload, dict)
        try:
            async with self._sessions() as session:
                session.add(
    ...
    @staticmethod
    def _claim_to_domain(raw: dict[str, Any]) -> RecommendationHumanReviewFindingClaim:
        payload = dict(raw)
        payload["claimed_at"] = datetime.fromisoformat(str(payload["claimed_at"]))
        return RecommendationHumanReviewFindingClaim(**cast(Any, payload))

    @staticmethod
    def _record_to_domain(raw: dict[str, Any]) -> RecommendationHumanReviewFindingRecord:
        payload = dict(raw)
        for field in ("created_at", "expires_at"):
            payload[field] = datetime.fromisoformat(str(payload[field]))
        return RecommendationHumanReviewFindingRecord(**cast(Any, payload))
