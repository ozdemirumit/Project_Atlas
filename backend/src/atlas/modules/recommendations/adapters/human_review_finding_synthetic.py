...
class SyntheticRecommendationHumanReviewFindingRecorder:
    def __init__(self, *, clock: Callable[[], datetime] | None = None) -> None:
        self._clock = clock or (lambda: datetime.now(UTC))
        self.calls: list[RecommendationHumanReviewFindingInstruction] = []
        self._artifacts: dict[str, tuple[RecommendationHumanReviewFindingItem, ...]] = {}
    ...
    async def record(
        ...
        receipt = RecommendationHumanReviewFindingReceipt(
            ...
            schema_version="atlas.recommendation-human-review-finding-receipt.v1",
            version=1,
            recorder_id="recommendation-human-review-finding-recorder.synthetic",
            attested_by="subject.recommendation-human-review-finding-recorder-attestor",
            ...
            finding_artifact_id=f"recommendation-review-finding-artifact.{seed}",
        ...
        if existing is not None and existing != instruction.findings:
            raise RecommendationHumanReviewFindingError(
                "recommendation_human_review_finding_artifact_conflict"
            )
    ...
    ) -> tuple[RecommendationHumanReviewFindingItem, ...] | None:
        return self._artifacts.get(finding_artifact_id)


class UnavailableRecommendationHumanReviewFindingRecorder:
    async def record(
        self, instruction: RecommendationHumanReviewFindingInstruction
    ) -> RecommendationHumanReviewFindingReceipt:
        del instruction
        raise RecommendationHumanReviewFindingError(
            "recommendation_human_review_finding_recorder_unavailable"
        )
