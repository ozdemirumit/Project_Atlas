...
class SyntheticStorageRecommendationAssembler:
    def build(
        self,
        ...
        *,
        ...
        evidence = {item.evidence_id for item in source_case.evidence}
        health = "evidence.investigation.health.latest"
        peer = "evidence.investigation.peer.signal"
        graph = "evidence.investigation.graph.path"
        vendor = "evidence.investigation.vendor.guidance"
        required = {health, peer, graph, vendor}
        ...
        applicability = Applicability(
            products=(model,),
            versions=("synthetic documented contract",),
            ...
            limitations=(
                "Synthetic lab applicability only.",
                "Revalidate after target, graph, health, policy, or RCA context changes.",
            ),
        )
        ...
        options = (
            ...
            self._escalate(request, applicability, impact, no_interruption, health, vendor),
            self._defer(request, applicability, impact, no_interruption, health, peer),
            self._restoration_plan(
                request,
                applicability,
                impact,
                blocked_interruption,
                health,
                graph,
                vendor,
            ),
    ...
    def _investigate(
        ...
        return RecommendationOption(
            ...
            supporting_evidence=(health, vendor),
            contradicting_evidence=(peer,),
        ...
        return RecommendationOption(
            ...
            plan_steps=(
                PlanStep(
                    ...
                    "Stop and escalate on critical state, peer degradation, or service symptom.",
                    False,
                ),
            ),
            supporting_evidence=(peer,),
            contradicting_evidence=(health,),
            assumptions=("Peer availability and service state do not deteriorate.",),
            unknowns=("Whether the warning will recur or worsen before the next observation.",),
            confidence="suspected",
            confidence_rationale=(
                "Deferral avoids change risk but accepts unresolved degradation risk."
            ),
            risk_dimensions=(
                RiskDimension(
            ...
            stop_conditions=(
                "Critical warning, peer degradation, service symptom, or stale evidence.",
            ),
            recovery=self._recovery(
    ...
    def _restoration_plan(
        ...
        return RecommendationOption(
            ...
            supporting_evidence=(health, graph, vendor),
            contradicting_evidence=(),
            assumptions=(),
