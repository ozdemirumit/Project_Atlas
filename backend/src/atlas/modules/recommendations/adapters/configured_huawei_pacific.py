...
from atlas.modules.connectors.vendors.huawei_pacific.manifest import (
    CLUSTER_FAILOVER_PLAN_CAPABILITY_ID,
    CLUSTER_NODE_CAPABILITY_ID,
    NODE_EVENTS_CAPABILITY_ID,
    NODE_REMEDIATION_PLAN_CAPABILITY_ID,
)
...
class ConfiguredHuaweiPacificRecommendationAssembler:
    """Builds the same five decision-support option categories as
    ConfiguredHuaweiDoradoRecommendationAssembler (investigate, escalate, defer, restoration
    planning, remediation planning) from a real `RcaCase` produced by
    ConfiguredHuaweiPacificRcaAssembler.

    The source case's evidence units carry generic source_type tags ("storage_hardware_health",
    "storage_inventory"), not vendor-specific ones, so this assembler's structure is otherwise
    identical to Dorado's -- only the vendor-facing capability ids and node/cluster wording
    differ. Needs no connector access itself; the source case is already real by the time it
    reaches here.
    """
    ...
    def build(
        ...
        *,
        ...
        impact = ImpactSummary(
            ...
            redundancy_effect="Reduced cluster redundancy is plausible but not confirmed.",
        ...
        blocked_interruption = InterruptionEstimate(
            ...
            assumptions=(),
            unknowns=(
                "Current redundancy, cluster rebalancing, maintenance-window, and service "
                "telemetry.",
            ),
        )
        options = (
    ...
    def _investigate(
        ...
        return RecommendationOption(
            ...
            title="Collect current node event and service evidence",
            intended_outcome=(
                "Distinguish persistent node degradation from a transient observation."
            ),
            applicability=applicability,
            plan_steps=(
                PlanStep(
                    "step.investigate.node-events",
                    1,
                    "diagnosis",
                    "Collect one bounded current node event snapshot.",
                    NODE_EVENTS_CAPABILITY_ID,
                    "C1",
                    "Current scoped node state and related events.",
                ...
                PlanStep(
                    ...
                    2,
                    "diagnosis",
                    "Repeat the exact target cluster-node observation.",
                    CLUSTER_NODE_CAPABILITY_ID,
                    "C1",
                    "A reproducible or absent finding on the same target.",
                    "Stop if the target or node mapping changes.",
                    False,
                ),
                PlanStep(
            ...
            unknowns=("Current node event, pool-capacity, and service state.",),
        ...
        return RecommendationOption(
            ...
            plan_steps=(
                PlanStep(
                    ...
                    1,
                    "monitoring",
                    "Continue the approved bounded cluster-node observation until expiry.",
                    CLUSTER_NODE_CAPABILITY_ID,
                    "C1",
            ...
            unknowns=("Whether the finding will recur or worsen before the next observation.",),
            confidence="suspected",
            confidence_rationale=(
                "Deferral avoids change risk but accepts unresolved degradation risk; no "
                "independent peer signal is available to weigh against the current finding."
            ),
            risk_dimensions=(
                RiskDimension(
                    "availability", RiskLevel.MODERATE, "Reduced cluster redundancy may persist."
                ),
            ...
            stop_conditions=("Critical finding, service symptom, or stale evidence.",),
            recovery=self._recovery(
    ...
    def _restoration_plan(
        ...
        return RecommendationOption(
            ...
            title="Prepare cluster node failover restoration planning",
    ...
    def _remediation_plan(
        ...
        return RecommendationOption(
            ...
            state=OptionState.BLOCKED,
            preference=PreferenceState.INELIGIBLE,
            title="Prepare permanent node or cluster remediation planning",
            intended_outcome=(
            ...
            plan_steps=(
                PlanStep(
                    ...
                    1,
                    "readiness",
                    "Validate the node mechanism with current bounded evidence.",
                    NODE_EVENTS_CAPABILITY_ID,
                    "C1",
                ...
                PlanStep(
                    ...
                    "Select an approved node or cluster remediation procedure.",
                    NODE_REMEDIATION_PLAN_CAPABILITY_ID,
                    "C3",
                    "A reviewed conceptual plan, not an executable command.",
            ...
            risk_dimensions=(
                RiskDimension(
                    "availability", RiskLevel.HIGH, "Remediation may disturb active nodes."
                ),
