...
_RCA_SEVERITY: dict[HuaweiPacificNodeRunningStatus, RcaSeverity] = {
    HuaweiPacificNodeRunningStatus.OFFLINE: RcaSeverity.CRITICAL,
    HuaweiPacificNodeRunningStatus.UNKNOWN: RcaSeverity.UNKNOWN,
}


def _identity(*parts: str) -> str:
    # Matches storage/graph/health_checks adapters' _identity scheme exactly, so an RCA
    # target_id agrees with the same real cluster's asset_id and graph entity_id.
    normalized = "\x1f".join(parts).encode("utf-8")
    return hashlib.sha256(normalized).hexdigest()[:20]


class ConfiguredHuaweiPacificRcaAssembler:
    """Builds a provisional RCA case from a real, single, current Huawei Pacific cluster-node
    read.

    Mirrors ConfiguredHuaweiDoradoRcaAssembler's shape and keeps the same two expert-authored
    fault-family hypothesis templates unchanged -- vendor-neutral storage reasoning content, not
    data. Two real differences from Dorado: (1) there is no confirmed cluster-level identifier or
    separate identity endpoint, so target resolution reads cluster inventory first and only then
    validates the request against the identity derived from the sorted set of node ids (matching
    the storage adapter's derivation exactly), rather than validating a known system_id up front;
    (2) both evidence units this assembler produces come from the single real cluster/servers
    read (node running status for "storage_hardware_health", node/cluster composition for
    "storage_inventory") because Pacific has no separate system-identity capability the way
    Dorado does.
    """
    ...
    async def build(
        ...
        *,
        ...
        if request.target_id != f"{_TARGET_PREFIX}{cluster_identity}":
            raise KeyError(request.target_id)
    ...
    def _assemble_case(
        ...
        *,
        ...
        non_online = tuple(
            ...
            if node.running_status is not HuaweiPacificNodeRunningStatus.ONLINE
        )
        health_evidence_id = (
            f"evidence.rca.health.{_identity(request.target_id, str(inventory.observed_at))}"
        )
        health_evidence = EvidenceUnit(
            ...
            collection_method="Huawei Pacific allowlisted C1 cluster-node read",
            summary=(
                f"{len(non_online)} non-online cluster node(s) out of {len(inventory.nodes)} "
                "observed."
            ),
            ...
            quality_limitations=(
                ...
                "No node event-log or peer-cluster evidence is included.",
            ),
            citation=f"Huawei Pacific cluster-node read for {len(inventory.nodes)} node(s).",
        )
        ...
        inventory_evidence = EvidenceUnit(
            ...
            authorization_reference=authorization_reference,
            collection_method="Huawei Pacific allowlisted C1 cluster-node read",
            summary=f"Confirmed cluster composition of {len(inventory.nodes)} node(s).",
            integrity="Digest-only evidence from an allowlisted C1 cluster-manager response",
            completeness="Node identity and membership only; no capacity detail is included.",
            quality_limitations=(
                "No confirmed cluster-level identifier exists; identity is derived from the "
                "sorted set of node ids.",
            ),
            citation=f"Huawei Pacific cluster-node read confirming {len(inventory.nodes)} node(s).",
        )
        timeline = (
            TimelineEvent(
                event_id=(
                    f"timeline.rca.{_identity(request.target_id, str(inventory.observed_at))}"
                ),
                event_type="storage_hardware_health_read",
                summary=health_evidence.summary,
                occurred_at=inventory.observed_at,
                observed_at=inventory.observed_at,
                ingested_at=inventory.observed_at,
        ...
        node = self._worst_node(non_online)
    ...
    @staticmethod
    def _worst_node(nodes: tuple[HuaweiPacificClusterNode, ...]) -> HuaweiPacificClusterNode:
        rank = {
            HuaweiPacificNodeRunningStatus.OFFLINE: 0,
            HuaweiPacificNodeRunningStatus.UNKNOWN: 1,
        }
        return min(nodes, key=lambda item: rank.get(item.running_status, 2))
    ...
    def _active_finding_case(
        ...
        *,
        ...
        prior_version_id: str | None,
        node_count: int,
        node: HuaweiPacificClusterNode,
        evidence: tuple[EvidenceUnit, ...],
        timeline: tuple[TimelineEvent, ...],
        health_evidence_id: str,
    ) -> RcaCase:
        component_label = f"node:{node.node_id}"
        severity = _RCA_SEVERITY.get(node.running_status, RcaSeverity.UNKNOWN)
        events_step = self._diagnostic_step(
            step_id="diagnostic.node-events",
            question="Is the finding reproduced by current node event evidence?",
            target_id=request.target_id,
            capability_id=NODE_EVENTS_CAPABILITY_ID,
            evidence_source="Huawei Pacific allowlisted read-only capability",
            expected_if_supported="Repeated node errors or a degraded node state are present.",
            expected_if_not_supported="No related current node error or event sequence is present.",
            supported_branch="Increase support for node or cluster degradation.",
        ...
        repeat_step = self._diagnostic_step(
            ...
            question="Does a repeat bounded cluster-node read reproduce the finding?",
            target_id=request.target_id,
            capability_id=CLUSTER_NODE_CAPABILITY_ID,
            evidence_source="Huawei Pacific allowlisted cluster-node capability",
            expected_if_supported="The same scoped node finding is reproduced.",
        ...
        hypotheses = (
            RcaHypothesis(
                ...
                statement=(
                    f"A node or cluster condition on {component_label} (running status "
                    f"'{node.running_status.value}') may be contributing to the finding."
                ),
                mechanism=(
                    "A faulty or unreachable node could reduce cluster redundancy and generate "
                ...
                expected_sequence=(
                    "Node condition begins.",
                    "The bounded cluster-node source observes a non-online running status.",
                    "Related node events may appear if the condition persists.",
                ),
                supporting_evidence=(health_evidence_id,),
                contradicting_evidence=(),
                missing_expected_observations=(
                    "Current node event-log evidence.",
                    "Current storage pool capacity and status.",
                ...
                assumptions=(
                    "The cluster-node response maps to the same target and node in the case scope.",
                ),
                confirmation_level=ConfirmationLevel.SUPPORTED,
                confidence_rationale=(
                    "A current, direct node finding and an applicable mechanism support the "
                    "hypothesis, but only one read was taken and no independent event or peer "
                    "evidence is available."
                ),
                diagnostic_steps=(events_step, service_step),
            ),
            RcaHypothesis(
                ...
                mechanism=(
                    "A short-lived state or collection limitation could produce a non-online "
                ...
                assumptions=("The connector's read reflects the cluster's true current state.",),
        ...
        return RcaCase(
            ...
            impact_scope=ImpactScope(
                ...
                current_impact=(
                    "A node finding is observed. No service outage is confirmed; reduced "
                    "cluster redundancy remains possible."
                ),
                ...
                limitations=(
                    ...
                    "No node event-log, pool-capacity, or application telemetry is available.",
                ),
            ),
            ...
            findings=(
                RcaFinding(
                    ...
                    cause_type=CauseType.CONTRIBUTING_CAUSE,
                    statement=(
                        "A node or cluster degradation mechanism is supported as a candidate "
                        "contributing condition, not a confirmed root cause."
                    ),
                    confirmation_level=ConfirmationLevel.SUPPORTED,
                    evidence_references=(health_evidence_id,),
                    residual_uncertainty=(
                        "No current node event-log, pool-capacity, or service evidence confirms "
                        "mechanism.",
                    ),
                ),
            ),
            assumptions=(
                "The connector's allowlisted read-only capability reflects the cluster's current "
                "state.",
            ),
            unknowns=(
                "Whether the finding is persistent or transient.",
                "Whether any downstream service is affected.",
                f"Whether the other {node_count - 1} node(s) in the cluster remain unaffected.",
            ),
            conflicts=(),
            evidence_gaps=(
                "Current node event-log evidence is missing.",
                "Current storage pool capacity and status evidence is missing.",
                "Current authorized application telemetry is missing.",
                "No authoritative recent-change record is linked.",
            ),
            blocker=(
                "Safe evidence is insufficient to distinguish persistent node degradation from a "
            ...
            provisional_statement=ProvisionalCauseStatement(
                statement=(
                    "No root cause is confirmed. Current evidence supports node or cluster "
                ...
                prevention_or_verification_implication=(
                    "Collect current node event, pool-capacity, and service evidence before "
                    "remediation or preventive-change planning."
                ),
            ),
            ...
            component_versions=(
                ...
                f"huawei-pacific-connector.{self._connector_version}",
            ),
        ...
        *,
        ...
        return RcaCase(
            ...
            impact_scope=ImpactScope(
                ...
                current_impact="No active node finding was observed for this target.",
            ...
            evidence_gaps=(
                ...
                "No node event-log or application telemetry is available.",
            ),
            blocker=(
                "No active node finding exists to investigate; the reported symptom may be "
            ...
            provisional_statement=ProvisionalCauseStatement(
                statement=(
                    "No root cause is confirmed. The current cluster-node read shows no active "
