"""Add bounded protected runtime process-resume request authorization.

Revision ID: 20260821_0154
Revises: 20260821_0153
"""
...
revision: str = "20260821_0154"
down_revision: str | None = "20260821_0153"
...
CONSUMPTION_CLAIM_TABLE = "workflow_event_runtime_process_scheduling_consumption_claims"
SCHEDULING_AUTH_LEASE_TABLE = "workflow_event_runtime_process_scheduling_auth_leases"
SCHEDULING_AUTH_CLAIM_TABLE = "workflow_event_runtime_process_scheduling_auth_claims"
LEASE_TABLE = "workflow_event_runtime_process_resume_auth_leases"
CLAIM_TABLE = "workflow_event_runtime_process_resume_auth_claims"
APPEND_ONLY_FUNCTION = "reject_wf_rtresume_auth_mutation"
SOURCE_POLICY_DIGEST = "138e8bbdf1472df0a9adfde7271b177d0d133f865d6e2548597a0b33143a6464"
    ...
    return (
        ...
        *(
            sa.Column(
                name,
                sa.String(64),
                nullable=name == "process_scheduling_failure_class",
            )
        ...
        sa.Column("protected_slot_generation", sa.Integer(), nullable=False),
        sa.Column("process_scheduling_outcome_known", sa.Boolean(), nullable=False),
        ...
        sa.Column("process_runnable", sa.Boolean(), nullable=False),
        ...
        sa.Column("process_scheduling_completed_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column(
            "process_scheduling_result_recorded_at", sa.DateTime(timezone=True), nullable=False
        ),
...
def _zero_authority(*, lease: bool) -> str:
    return " AND ".join(
        ("" if lease and name == "protected_runtime_process_resume_authority_granted" else "NOT ")
        + name
...
def _contract_check() -> str:
    return (
        ...
        "AND purpose_id = 'purpose.workflow-protected-runtime-process-resume-request' "
        "AND policy_id = 'policy.workflow-protected-runtime-process-resume-authorization' "
...
def _source_check() -> str:
    return (
        "process_scheduling_result_state = 'process_scheduled_suspended_in_protected_boundary' "
...
def _source_constraints(prefix: str) -> tuple[sa.ForeignKeyConstraint, ...]:
    return (
        ...
        sa.ForeignKeyConstraint(
            ATTEMPT_LOCAL,
            [f"{ATTEMPT_TABLE}.{name}" for name in ATTEMPT_REMOTE],
        ...
        sa.ForeignKeyConstraint(
            ...
            tuple(
                f"{SCHEDULING_AUTH_LEASE_TABLE}.{name}"
                for name in (
                    *SCOPE,
        ...
        sa.ForeignKeyConstraint(
            ...
            tuple(
                f"{SCHEDULING_AUTH_CLAIM_TABLE}.{name}"
...
def _common_constraints(prefix: str) -> tuple[sa.Constraint, ...]:
    return (
        *_source_constraints(prefix),
        sa.CheckConstraint(_contract_check(), name=f"ck_wf_rtresume_{prefix}_contract"),
        sa.CheckConstraint(_source_check(), name=f"ck_wf_rtresume_{prefix}_source"),
    )
...
def _create_lease() -> None:
    op.create_table(
        ...
        sa.Column("process_state_eligible_until", sa.DateTime(timezone=True), nullable=False),
        sa.Column("attestation_metadata_only", sa.Boolean(), nullable=False),
        sa.Column("resume_profile_id", sa.String(128), nullable=False),
        sa.Column("resume_profile_version", sa.String(64), nullable=False),
        sa.Column("resume_profile_digest", sa.String(64), nullable=False),
        ...
        *_identity_columns(),
        *_authority_columns(),
        *_common_constraints("lease"),
        sa.UniqueConstraint("process_scheduling_result_id", name="uq_wf_rtresume_lease_result"),
        sa.UniqueConstraint("claim_id", name="uq_wf_rtresume_lease_claim"),
        sa.UniqueConstraint(
            *SCOPE,
            "authorization_lease_id",
            "claim_digest",
    ...
    op.create_index("ix_wf_rtresume_lease_scope", LEASE_TABLE, [*SCOPE, "issued_at"])
...
def _create_claim() -> None:
    op.create_table(
        ...
        *_common_constraints("claim"),
        ...
        sa.UniqueConstraint("process_scheduling_result_id", name="uq_wf_rtresume_claim_result"),
        sa.UniqueConstraint("authorization_lease_id", name="uq_wf_rtresume_claim_lease"),
        sa.UniqueConstraint(
            *SCOPE,
            "claim_id",
            "canonical_digest",
    ...
    op.create_index("ix_wf_rtresume_claim_scope", CLAIM_TABLE, [*SCOPE, "claimed_at"])


def upgrade() -> None:
    op.create_unique_constraint(
        "uq_wf_rtresume_src_sched_result", RESULT_TABLE, list(RESULT_REMOTE)
    )
    op.create_unique_constraint(
        "uq_wf_rtresume_src_sched_attempt", ATTEMPT_TABLE, list(ATTEMPT_REMOTE)
    )
    op.create_unique_constraint(
        "uq_wf_rtresume_src_sched_claim",
    ...
    op.create_foreign_key(
        "fk_wf_rtresume_lease_auth_claim",
        LEASE_TABLE,
        CLAIM_TABLE,
    ...
    op.execute(
        sa.text(
            ...
            BEGIN
                RAISE EXCEPTION 'protected process-resume authorization evidence is append-only';
            END;
            $$
            """
        )
    ...
    for table, stem in ((LEASE_TABLE, "lease"), (CLAIM_TABLE, "claim")):
        op.execute(
            sa.text(
                f"CREATE TRIGGER trg_wf_rtresume_{stem}_append_only "
        ...
        op.execute(
            sa.text(
                f"CREATE TRIGGER trg_wf_rtresume_{stem}_no_truncate "
...
def downgrade() -> None:
    ...
    if lease_count or claim_count:
        raise RuntimeError(
            "refusing guarded downgrade: protected runtime process-resume authorization evidence exists"
        )
    op.drop_constraint("fk_wf_rtresume_lease_auth_claim", LEASE_TABLE, type_="foreignkey")
    op.drop_table(CLAIM_TABLE)
    op.drop_table(LEASE_TABLE)
    op.drop_constraint("uq_wf_rtresume_src_sched_claim", CONSUMPTION_CLAIM_TABLE, type_="unique")
    op.drop_constraint("uq_wf_rtresume_src_sched_attempt", ATTEMPT_TABLE, type_="unique")
    op.drop_constraint("uq_wf_rtresume_src_sched_result", RESULT_TABLE, type_="unique")
