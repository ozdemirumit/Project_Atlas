"""Add immutable MCP Builder independent security reviews.
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("domain_review_profile", sa.String(length=128), nullable=False),
        sa.Column("domain_reviewer_contract_version", sa.String(length=128), nullable=False),
        ...
        sa.Column("control_assessments", postgresql.JSONB(), nullable=False),
        sa.Column("accepted_count", sa.Integer(), nullable=False),
        sa.Column("needs_remediation_count", sa.Integer(), nullable=False),
        ...
        sa.CheckConstraint("version = 1", name="ck_mcp_builder_security_reviews_version"),
        sa.PrimaryKeyConstraint("review_id"),
        sa.UniqueConstraint("project_id", name="uq_mcp_builder_security_reviews_project"),
        sa.UniqueConstraint(
            "domain_review_id", name="uq_mcp_builder_security_reviews_domain_review"
        ),
        sa.UniqueConstraint(
            "reviewed_by",
            "idempotency_key",
    ...
    for column in (
        ...
        op.create_index(
            op.f(f"ix_mcp_builder_security_reviews_{column}"),
            "mcp_builder_security_reviews",
            [column],
            unique=False,
        )
...
def downgrade() -> None:
    for column in reversed(
        ...
        op.drop_index(
            op.f(f"ix_mcp_builder_security_reviews_{column}"),
            table_name="mcp_builder_security_reviews",
        )
    op.drop_table("mcp_builder_security_reviews")
