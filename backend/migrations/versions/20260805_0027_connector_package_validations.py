"""Add immutable connector package validation intake reports.
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("manifest_path", sa.String(length=300), nullable=False),
        sa.Column("manifest_digest", sa.String(length=64), nullable=True),
        sa.Column("capability_ids", postgresql.JSONB(), nullable=False),
        sa.Column("schema_evidence", postgresql.JSONB(), nullable=False),
        ...
        sa.CheckConstraint("version = 1", name="ck_connector_package_validations_version"),
    ...
    for column in (
        ...
        op.create_index(
            op.f(f"ix_connector_package_validations_{column}"),
            "connector_package_validations",
            [column],
            unique=False,
        )
...
def downgrade() -> None:
    for column in reversed(
        ...
        op.drop_index(
            op.f(f"ix_connector_package_validations_{column}"),
            table_name="connector_package_validations",
        )
    op.drop_table("connector_package_validations")
