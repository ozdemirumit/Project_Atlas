...
- IMP-186 provides durable fenced ownership but intentionally creates no execution instance. The
  next smallest durable boundary is one immutable `created` run and its ordered `not_started`
  step-run records, derived from the exact code-owned definition.
- Only the exact active lease holder may materialize a run. The operation revalidates plan, lease,
  scope, target, worker, digest and fencing token and atomically persists one idempotent result.
- Attempt creation, queue delivery, worker dispatch, running states, retries, timers, signals,
  connector/model calls, approvals, ITSM, runbooks and infrastructure actions remain deferred.

### ATLAS-IMP-187 Acceptance Criteria

- A dedicated workflow workload identity can idempotently materialize exactly one immutable run for
  an exact `planned` plan under its active unexpired lease; competing or changed requests fail
  closed without revealing unauthorized state.
- The run remains `created`; ordered step runs remain `not_started` and preserve step kind,
  capability, timeout and dependency contracts from the exact definition version. Every authority
  field remains structurally false.
- PostgreSQL locks and revalidates the plan and lease, then writes run, step runs and the immutable
  idempotency claim in one transaction. Production never falls back to memory.
- Human UI exposes authoritative read-only run state with no materialize, dispatch or attempt
  controls, no credential material, no second-login or MFA prompt and no implication that work ran.

### ATLAS-IMP-187 Validation Evidence

- Domain and application services create one deterministic immutable `created` run with ordered
  `not_started` step runs only after exact plan, definition, lease, workload identity, scope,
  target, digest and fencing checks. Exact idempotent replay returns the same record; stale,
  released, expired, cancelled, changed or competing requests fail closed.
- PostgreSQL locks and revalidates the plan and current lease, then persists run, ordered steps and
  the immutable idempotency claim atomically. Historical lease evidence is preserved without an
  unsafe foreign key to the replaceable current-lease row. Alembic has one head at
  `20260814_0110`.
- API tests prove that workload-token identity is required for materialization while the existing
  username/password browser session can read the result without MFA or another authorized-browser
  prompt. Browser mutation receives `401`; every execution-authority field remains false and the
  original plan is unchanged.
- Ruff format/lint and strict mypy passed across 1,018 source files. The full backend suite passed
  1,612 tests with three expected Windows symlink skips. The full frontend suite passed 305 tests
  across 95 files; ESLint, TypeScript and the production build passed with only the pre-existing
  chunk-size advisory.
- Live validation at `http://127.0.0.1:5253/#/workspace/workflows` restarted the backend from this
  branch and used one `atlas-demo` / `local-demo` username/password login. Plan
  `workflow-plan.42814d840be192e2d9c16fda` showed `Materialized run record`, `No human controls`
  and no run yet, without a second login or MFA prompt. The UI continued to state that no
  connector, approval, ITSM, runbook, worker or infrastructure action ran.

### ATLAS-IMP-186 Delivery Evidence

- Source head `25b84f233540a849dc493b8b2871011bb6f48dfd` passed exact-head PR CI run
  `31743926312`; frontend completed in 4m59s and backend in 8m50s.
- PR [#198](https://github.com/ozdemirumit/Project_Atlas/pull/198) was squash-merged as
  `4dcca330090a76d1bd56c90d8ee7fc8e01e98b35`.
- The exact merged commit independently passed `main` CI run `31744701149`; frontend completed in
  4m54s and backend in 9m39s. Local `main` was fast-forwarded to the same verified commit before
  IMP-187 branched.
...
- IMP-160 makes provider and key trust posture observable, but metadata alone does not prove that the
  configured provider can currently sign and verify an exact-scope payload through its adapter.
- Concrete KMS/HSM selection still needs deployment input. A provider-neutral conformance assessment
  can validate the existing port contract before vendor onboarding without accepting caller payload,
  exposing a signature or changing key lifecycle state.
- The assessment is security diagnostic evidence only. It cannot create a receipt, approve an upgrade,
  manage a key, contact infrastructure or grant signing, handoff, runtime or execution authority.

### ATLAS-IMP-161 Acceptance Criteria

- A versioned assessment binds exact organization/environment, provider class, key reference,
  algorithm, server-generated challenge digest, policy version, observation time and bounded expiry.
- A dedicated provider-neutral diagnostic invokes sign and verify only for one server-generated inert
  challenge; caller-shaped payloads, signatures, key references and provider parameters are forbidden.
- Assessment distinguishes conformant, unavailable, ineligible-key, sign-failed, verify-failed and
  policy-blocked outcomes with stable reason codes and secret-free evidence. Raw signature values,
  key material, credentials, tokens and endpoints never leave the adapter boundary.
- Authenticated exact-scope C2 create and C1 latest-read APIs use dedicated default-deny RBAC, browser
  session plus CSRF for creation, no-store responses, idempotency and required attributable audit.
- The Connector inventory UI can run and inspect the bounded assessment, clearly labels production
  approval and limitations, and exposes no key-management, receipt-signing or operational controls.
- ADR, focused and full backend/frontend gates, one Alembic head, production build and live responsive
  validation pass before delivery.

### ATLAS-IMP-161 Initial Evidence

- IMP-160 merged through PR #172 as `6718a34dd39671147b20f7815384ba19a01300e0`; merged-main CI run
  `31597991161` passed with backend in 6m54s and frontend in 4m46s.
- ADR-116 exposes immutable trust metadata but explicitly leaves provider onboarding and lifecycle
  mutation for later decisions. The existing provider port already offers exact-scope sign and verify
  operations suitable for a bounded conformance challenge.
- No approved production KMS/HSM provider is available in this workspace, so production remains
  unavailable and fail-closed while the non-production adapter supplies deterministic test evidence.
...
- Source commit `4ab543c` is published in draft PR
  [#172](https://github.com/ozdemirumit/Project_Atlas/pull/172). Clean branch CI run
  `31596772931` passed at `324fad863b61c6d55ede1a8ccd667390881da43e` with backend in 6m05s and
  frontend in 4m37s. Final exact-head run `31597334253` passed at
  `25d10b921c0ab307564f3e1b04315b5fdbe78a1b` with backend in 7m33s and frontend in 4m42s.
- PR #172 was squash-merged as `6718a34dd39671147b20f7815384ba19a01300e0`; merged-main run
  `31597991161` passed with backend in 6m54s and frontend in 4m46s.
...
- IMP-152 correctly leaves ITSM, maintenance-window and audit-readiness evidence blocked.
- The existing platform change-review contract cannot safely represent connector package lineage.
- ADR-109 introduces an exact-readiness-bound internal draft while deliberately leaving external
  ITSM dispatch, approval, window authority and blocker resolution out of scope.

### ATLAS-IMP-153 Acceptance Criteria

- The latest independent verifier can create one immutable, idempotent draft bound to the exact
  current readiness digest and a bounded proposed window.
- Stale readiness, invalid windows, missing acknowledgement, cross-scope access and conflicting
  idempotent replay fail closed.
- PostgreSQL persistence is append-only with a unique actor-idempotency constraint and migration
  head remains linear.
- Create/latest-read endpoints are CSRF/RBAC protected, no-store, audit recorded and omit internal
  fingerprints, idempotency keys, secrets, endpoints and credentials.
- The UI creates and restores the draft and clearly displays no dispatch, no window approval and
  continued blocked handoff state.
- ITSM and maintenance-window blockers remain unresolved; no handoff, execution or infrastructure
  authority is created.
- Focused and full backend/frontend gates, production build and live responsive validation pass.

### ATLAS-IMP-153 Initial Evidence

- IMP-152 merged through PR #164 as `2d7e7fc98f6e7c2d73659303618ac3b815be95ba`;
  tracker closure `e35ad1009cb32d26345eceb3be42e9de5ee042d9` passed main CI run
  `31559207360` before IMP-153 branched.
- ATLAS-036 keeps external ITSM authoritative and prohibits interpreting a draft or ticket state as
  unrestricted change authority.
- ADR-108 leaves ITSM, window and audit readiness blocked and requires separate evidence contracts.

### ATLAS-IMP-153 Validation Evidence

- The immutable draft binds exact request, decision, revalidation and handoff-readiness digests,
  rejects stale/expired evidence, enforces independent-verifier ownership and records create, replay
  and read audit outcomes. ITSM dispatch, window approval, handoff, target contact, package rebinding,
  configuration mutation, approval consumption, execution and infrastructure mutation remain false.
- Backend formatting and lint passed 1,185 files, mypy passed 1,083 source files, and the complete
  backend suite passed 977 tests with three environment-specific Windows symlink skips. Alembic
  remains at the single linear `20260812_0100` head.
- Frontend ESLint and both TypeScript projects passed; all 217 tests in 88 files passed. The
  2,007-module production build completed with the existing advisory limited to the transitional
  `OperationalApplication` chunk.
- Live validation created the internal draft and rendered its no-dispatch, no-window-approval and
  blocked-handoff state at 1,280 px and 390 px. Both widths had zero horizontal document overflow,
  mobile scroll X remained zero, the dialog measured 390 x 844 px, no browser errors occurred and
  no install, apply, execute or handoff control existed. The off-canvas mobile sidebar was the only
  intentionally out-of-viewport element group.
...
### ATLAS-IMP-149 Delivery Evidence

- Implementation commit `1b721abccd34b9166e34368bfdba707208b14230` passed exact-head
  pull-request CI run `31547490870`; backend completed in 7m13s and frontend in 4m32s.
- PR [#161](https://github.com/ozdemirumit/Project_Atlas/pull/161) was squash-merged to `main` as
  `6109a67efcc39315934b1eb1ec2460cb51c3adda`.
- The exact merged `main` commit passed independent push CI run `31547983912`; backend completed in
  7m16s and frontend completed in 4m23s with every required step successful.
...
- Production Vite build transformed 2,000 modules and emitted a separate 8.22 KB Data
...
- An isolated Vite harness rendered the real component and production CSS without submitting Data
  Initialization. Desktop review showed the complete bounded dialog, two ordered reversible
  migrations, disabled confirmation and no overlap.
- Exact CDP emulation at 390 x 844 measured document width at 390/390 px, the dialog at 358 px with
  a 374 px right edge, and the input and both buttons at 328 px. Confirmation began disabled and no
  database URL, SQL, password or secret text was present. Cancel removed the dialog, restored the
...
- Merged-main CI run `31491633235` passed frontend and backend jobs. Local `main` was fast-forwarded
  to the verified merge commit before the tracker closure.
- Tracker closure commit `d18ddf20d1d654d6761a833c399176103f983262` passed independent main CI
  run `31492531790`, including frontend and backend jobs.
...
- Mobile validation passed at a 375 CSS-pixel viewport: the document remained 375/375 px and the
  ...
  isolated iframe emitted only the known Browser instrumentation `MutationObserver` error; direct
  Atlas pages were clean.
- PR #143 passed Continuous Integration run `31416071298` with successful frontend and backend
  jobs, then squash-merged as `9afead6caed5acd16dafbe947b03dc5bfbc37346`.
- The merged `main` commit passed independent push CI run `31416782075` with successful frontend and
  backend jobs.
...
- IMP-117 permits the exact assigned reviewer to inspect one redacted recommendation snapshot but
  deliberately records no human observation or decision.
- ADR-074 permits one immutable encrypted finding packet for that exact presentation while the
  lease, assignee, browser and selected track remain current.
- Findings are accountable evidence only. Decisions, correction, approval, workflow, ITSM and
  every operational capability remain independent later stages.

### ATLAS-IMP-118 Acceptance Criteria

- Only an active integrity-valid presentation with unchanged complete recommendation lineage may
  enter finding recording; exact hardware-MFA assignee, browser binding and track cookie remain
  mandatory.
- The caller supplies one to twenty policy-catalog findings, exact presentation/policy digests,
  purpose and no-authority acknowledgements. Identity, track, source content, policy catalogs,
  artifact controls, decision, approval, workflow, command and mutation fields are forbidden.
- Technical and service-impact category catalogs are distinct and policy owned. Cross-track
  categories, unknown severity, active payloads and excess bounds fail closed before persistence.
- One immutable claim exists per presentation. A trusted recorder writes encrypted immutable
  content and returns only a signed receipt; production fails closed without that recorder.
- Plaintext findings never enter PostgreSQL, API responses, audit, logs, cookies, browser storage,
  model context, retrieval or graph state. Only minimized metadata and digests are exposed.
- Success records findings only and leaves review completion, disposition, correction, approval,
  workflow, ITSM, execution, deployment and mutation false.
- Memory/PostgreSQL parity, one Alembic head, strict schemas, default-deny C2 create/C1 read RBAC,
  CSRF, focused lineage/track/replay/privacy/audit tests, bounded UI, full suites, live validation
  and GitHub CI apply.

### ATLAS-IMP-118 Initial Evidence

- IMP-117 merged through PR #129 as `b5dfbd269fb6d559ce6af4d5ada0751817f964a5`; PR run
  `31346338749` and merged-main run `31346643336` passed both frontend and backend jobs.
...
- Implementation commit `e158a8b` and tracker commit `3a45f0b` merged through PR #130 as
  `8d13187a12e91706bb86310dd5cceecf7b5ebf60`.
...
- The service rehydrates and verifies the exact promotion, readiness assessment, review request,
  reviewer assignment, inspection lease and protected presentation before accepting one to twenty
  track-catalog findings. Wrong recommendation paths, cross-track categories, permission denial,
  required intent-audit failure and unavailable recorder state fail closed.
- One immutable claim is permitted per exact presentation. Exact replay returns only the same
  metadata record; the synthetic development recorder seals finding plaintext outside the
  application record and production uses an unavailable recorder. PostgreSQL, API schemas, audit,
  cookies and browser state expose digests and bounded counts but no finding summary or detail.
- Technical and service-impact catalogs remain distinct. Success sets only the matching finding
  flag while human review completion, recommendation approval, correction, workflow, ITSM,
  execution, deployment and infrastructure mutation remain false.
- The HTTPS API flow passed login, exact lease-cookie binding, protected presentation, CSRF denial
  and successful metadata-only finding creation. Responses use `no-store`, restrictive CSP and no
  plaintext finding fields. The React client validates exact recommendation lineage recursively,
  sends CSRF and rejects plaintext or authority-bearing responses.
- Alembic revision `20260810_0090` is the single head. Offline PostgreSQL SQL creates separate
  immutable claim and finding metadata tables with unique presentation constraints and no plaintext
  finding columns.
- Ruff formatting and lint passed across `1110` Python files. Strict mypy found no issues in `1019`
  source and test files. The complete backend suite passed `925` tests with three expected Windows
  symlink skips and three existing dependency warnings.
- Frontend TypeScript and ESLint passed. The complete Vitest suite passed `90` tests across `56`
  files and the production bundle built successfully. Focused recommendation content/finding and
  lifecycle coverage passed `5` tests.
- Live API and web processes ran on `127.0.0.1:8082` and `127.0.0.1:5242`. Desktop inspection showed
  the new capability as the latest available stage; the 390-pixel viewport retained the capability
  and reported no page-level horizontal overflow.
- PR run `31350896832` passed frontend in 4m19s and backend in 6m30s. The merged-main run
  `31351222388` passed frontend in 4m32s and backend in 6m25s.
...
- Scope commit `dac9aa5`, implementation commit `d6d8ee0` and PR-link commit `30aa129` contain the
  authored slice. PR #127 branch run `31339863831` passed (frontend 2m34s, backend 4m21s). The PR
  was squash-merged as `b23bfde8cc1944d066df0e7bcb4717da603dca74`; merged-main run
  `31340062662` passed (frontend 4m16s, backend 5m45s). IMP-115 is complete and the
...
- IMP-110 establishes an exact protected preference, tie or no-support outcome but deliberately
  exposes only aggregate counts and digests.
- ADR-067 permits the same eligible human consumer to cross one accountable deterministic
  presentation boundary and receive inert, bounded, evidence-linked decision support.
- A preferred result may present one exact candidate, alternatives, impact, estimated work and
  interruption, recovery, gaps and unknowns. Tie and no-support outcomes must be presented without
  inventing a selection.
- Existing-domain promotion, review readiness, approval, workflow, ITSM, execution, deployment and
  infrastructure mutation remain later stages.

### ATLAS-IMP-111 Acceptance Criteria

- Only one exact completed, unexpired, integrity-valid adjudication with unchanged candidate,
  impact, completion, evidence, graph, answer, invocation, context, retrieval, source, policy,
  purpose, browser, consumer and protected-vault lineage can enter presentation.
- The caller supplies only exact adjudication and signed presentation-policy bindings, unchanged
  purpose, three acknowledgements, idempotency and correlation. Candidate, outcome, content,
  ordering, impact, risk, duration, recovery, target, workflow, operation and mutation controls are
  forbidden.
- One signed immutable policy defines allowed fields, outcome preservation, inert rendering,
  redaction, classification, count, character, byte and retention limits.
- A trusted deterministic no-network, no-model presenter preserves preferred, tie and no-support
  outcomes exactly and emits bounded source-derived content with conservative impact, risk,
  duration, recovery, gaps and unknowns.
- Raw candidates, graph paths, comparison values, protected reports, evidence payloads and receipt
  remain in a tenant-isolated vault. Ordinary persistence/API/UI receive only redacted presented
  content and minimized lineage, outcome, counts, digests, timestamps and safety flags.
- One immutable claim prevents multiple presentations. Exact replay verifies and returns the same
  content after current authorization and complete source, policy, retention, vault and digest
  checks and never re-renders changed inputs.
- Success sets recommendation presentation only. Existing-domain promotion, review readiness,
  approval, workflow, ITSM, execution, deployment and infrastructure mutation remain false.
- Memory/PostgreSQL parity, one Alembic head, strict schemas, default-deny RBAC, CSRF, focused
  preferred/tie/no-support/lineage/replay/redaction/privacy/vault/audit tests, bounded UI, full
  suites, live desktop/mobile validation and GitHub CI apply.

### ATLAS-IMP-111 Validation Evidence
