...
| Related | ATLAS-001, ATLAS-002, ATLAS-003, ATLAS-010, ATLAS-016, ATLAS-032, ATLAS-036, ATLAS-037, ATLAS-047, ATLAS-050, ATLAS-052, ATLAS-055, ATLAS-056, ADR-123, ADR-133 |
...
The reviewer must be a separately attributable human using an authenticated browser session and the
dedicated `role.itsm-reviewer` role. Development authentication and single-factor assurance are
valid by default. In accordance with ADR-133, multi-factor or hardware-backed assurance may be
required only by an explicit deployment policy; neither is a hard-coded prerequisite of this
workflow. Exact-scope C1 read and C2 decision permissions remain default deny. Self-review, changed
or expired source material, wrong scope, missing acknowledgement, invalid rationale and conflicting
idempotency replay fail closed. An exact retry returns the original immutable record.
...
- Separation of duties, human identity, exact-scope RBAC and attributable audit prevent the report
  requester from self-attesting without imposing a fixed MFA requirement.
...
- Domain, service, API authorization, single-factor development session, CSRF, idempotency,
  separation and PostgreSQL mapping tests
