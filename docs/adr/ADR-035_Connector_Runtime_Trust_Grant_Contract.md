...
ADR-133 supersedes this ADR's fixed MFA and minimum-assurance prerequisite. Runtime-trust grant
still requires a dedicated, authenticated, authorized human and every existing RBAC, exact-scope,
acknowledgement, separation, signed-policy, lineage/freshness, idempotency, audit, runtime-isolation,
and no-execution control. The default policy requires no MFA and uses `SINGLE_FACTOR`; an optional
named step-up policy may add assurance checks only when explicitly configured, but assurance is not
authorization.
...
Only a dedicated, authenticated, authorized, exact-tenant human with C3 permission may grant
runtime trust. The actor must be distinct from every upstream actor and profile/policy signer. AI,
service, shared, wrong-scope, unauthorized identities, and identities that do not satisfy the
optional configured `connector_runtime_trust` step-up policy fail closed without discovery. The
signed runtime-trust policy defaults to `SINGLE_FACTOR`; it defines accepted external assurance
values and freshness only when that named step-up policy is explicitly configured.
...
the optional named `connector_runtime_trust` step-up policy only when configured.
